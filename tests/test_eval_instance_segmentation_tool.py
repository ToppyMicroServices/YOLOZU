...
class TestEvalInstanceSegmentationTool(unittest.TestCase):
    def _build_fixture(self, root: Path):
        ...
        return dataset_root, split, preds_root, pred_json
    ...
    def test_eval_instance_segmentation_smoke_with_html_and_overlays(self):
        ...
        with tempfile.TemporaryDirectory(dir=str(repo_root)) as td:
            root = Path(td)
            dataset_root, split, preds_root, pred_json = self._build_fixture(root)
    ...
    def test_eval_instance_segmentation_tolerates_invalid_classes_json(self):
        repo_root = Path(__file__).resolve().parents[1]
        script = repo_root / "tools" / "eval_instance_segmentation.py"
        self.assertTrue(script.is_file())
        ...
        with tempfile.TemporaryDirectory(dir=str(repo_root)) as td:
            ...
            classes_json.write_text("{invalid json", encoding="utf-8")
            out_json = root / "instance_seg_eval.json"
            ...
            proc = subprocess.run(
                [
                    ...
                    str(preds_root.relative_to(repo_root)),
                    "--classes",
                    str(classes_json.relative_to(repo_root)),
                    "--output",
                    str(out_json.relative_to(repo_root)),
                ],
                cwd=str(repo_root),
            ...
            if proc.returncode != 0:
                self.fail(f"eval_instance_segmentation.py failed with invalid classes json:\n{proc.stdout}\n{proc.stderr}")

            report = json.loads(out_json.read_text(encoding="utf-8"))
            classes = (report.get("metrics") or {}).get("classes") or []
            self.assertEqual([row.get("name") for row in classes], [None, None])
