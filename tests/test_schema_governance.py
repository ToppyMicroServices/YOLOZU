...
class TestSchemaGovernance(unittest.TestCase):
    def test_adaptive_routing_schema_copies_and_browser_are_current(self):
        ...
        expected = {
            ...
            "ocr_bundle_interface_json": "ocr_bundle_interface.schema.json",
            "ocr_result_json": "ocr_result.schema.json",
            "frame_result_json": "frame_result.schema.json",
            "stream_job_spec_json": "stream_job_spec.schema.json",
            "stream_workload_profile_json": "stream_workload_profile.schema.json",
            "stream_summary_json": "stream_summary.schema.json",
            "stream_qualification_report_json": "stream_qualification_report.schema.json",
            "stream_selection_decision_json": "stream_selection_decision.schema.json",
            "tracking_output_interface_json": "tracking_output_interface.schema.json",
            "tracking_output_record_json": "tracking_output_record.schema.json",
    ...
    def test_adaptive_routing_record_instances_have_one_packaged_ssot(self):
        ...
        screening_lines = [
            json.loads(line)
            for line in (data_root / "candidate_screening.jsonl")
            .read_text(encoding="utf-8")
            .splitlines()
        ]
        self.assertEqual(len(screening_lines), 2)
        self.assertEqual(
            {record["overall_status"] for record in screening_lines}, {"hold"}
        )
        self.assertEqual(
