...
- `.github/workflows/container.yml` (**optional publish**): expected to build on pull requests for container-related changes and publishes only on tag/manual; manual reruns should pass `release_tag=vX.Y.Z`, and NGC mirrors use `nvcr.io/yolozu/...` when `NGC_API_KEY` is configured.
