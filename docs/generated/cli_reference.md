...
| cancel_image_job | experimental | yolozu/integrations/mcp_server.py | Cancel one queued image job and release its retained asset reference; apply tenant rate and retention limits without interrupting a running model. |
...
| run_mcp_server | stable | tools/run_mcp_server.py | Run the YOLOZU MCP server over stdio or Streamable HTTP with optional bounded image-service tools, tenant rate limits, and idle-time retention cleanup. |
...
| submit_image_job | experimental | yolozu/integrations/mcp_server.py | Rate-limit and queue one typed image job using only a qualified registered pipeline or abstention; execution is opt-in and retained assets are released on completion or queued cancellation. |
