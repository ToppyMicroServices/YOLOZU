...
| run_mcp_server | stable | tools/run_mcp_server.py | Run the YOLOZU MCP stdio server, expose canonical live tool ids, and inspect filtered packaged discovery with exact generated input schemas, explicit unclassified metadata, and guaranteed AI-safe/config-review/Actions surface sets. |
