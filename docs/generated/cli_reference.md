...
| cancel_image_job | experimental | yolozu/integrations/mcp_server.py | Cancel one queued image job without claiming interruption of an already running model process. |
...
| get_image_job | experimental | yolozu/integrations/mcp_server.py | Return bounded status and result data for one opaque image job without exposing internal paths. |
...
| image_service_capabilities | experimental | yolozu/integrations/mcp_server.py | Describe the bounded single-tenant image-service interface contract without selecting or executing a model. |
...
| put_image_asset | experimental | yolozu/integrations/mcp_server.py | Decode, validate, and privately store one bounded image without accepting a filesystem path or remote URL. |
...
| run_mcp_server | stable | tools/run_mcp_server.py | Run the YOLOZU MCP server over stdio or Streamable HTTP, optionally restrict it to the bounded image-service surface, and expose canonical manifest-backed discovery. |
...
| submit_image_job | experimental | yolozu/integrations/mcp_server.py | Queue one typed image job that uses only a qualified registered pipeline or explicitly abstains; execution is opt-in. |
