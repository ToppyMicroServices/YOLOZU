...
| cancel_image_job | experimental | yolozu/integrations/mcp_server.py | Cancel queued image work or stop a running owned process; release retained assets after termination and preserve tenant rate limits. |
...
| export_bop19_rtdetr_pose | research | tools/export_bop19_rtdetr_pose.py | Export trained RT-DETR object-pose predictions over official BOP19 targets without using test GT during inference. |
...
| export_predictions_ultralytics | experimental | tools/export_predictions_ultralytics.py | Run identity-safe Ultralytics inference and export strict YOLOZU predictions JSON without losing source paths in current runtime batching. |
...
| generate_web_docs | stable | tools/generate_web_docs.py | Safely generate searchable web docs and LLM discovery/onboarding files from repository-local SSOT sources, schemas, and curated evidence. |
| get_image_job | experimental | yolozu/integrations/mcp_server.py | Return bounded status and result data for one opaque image job, enforcing tenant polling and expiry limits without exposing internal paths. |
...
| prepare_bop_yolozu | research | tools/prepare_bop_yolozu.py | Convert BOP rigid-object pose data into an owned YOLOZU dataset with deterministic splits, strict GT sidecars, metre-scaled CAD points/keypoints, and provenance. |
...
| put_image_asset | experimental | yolozu/integrations/mcp_server.py | Rate-limit, decode, validate, and privately store one bounded image without accepting a filesystem path or remote URL. |
...
| run_external_finetune_smoke | experimental | tools/run_external_finetune_smoke.py | Run external finetune smoke matrix for YOLOX/Ultralytics/MMDetection/Detectron2/RT-DETR; non-dry projection-only lanes fail closed, while the report records dependency status, artifact hashes, wall time, peak RSS, runtime, source, and license boundaries. |
| run_external_runtime_gpu_qualification | experimental | scripts/run_external_runtime_gpu_qualification.sh | Install pinned YOLOX and OpenMMLab runtimes and execute all non-dry training lanes on a compatible Linux CUDA host while retaining structured failures and requiring checkpoints, launcher resource use, and structural resume/export/eval/parity handoff validation. |
| run_mcp_server | stable | tools/run_mcp_server.py | Run the YOLOZU MCP server over stdio or Streamable HTTP with bounded image-service tools, exclusive tenant ownership, cancellable process jobs, rate limits, and idle-time retention cleanup. |
...
| submit_image_job | experimental | yolozu/integrations/mcp_server.py | Queue a deadline-bounded cancellable image process using only a qualified registered pipeline or abstention; execution is opt-in and terminal paths release retained assets. |
...
| summarize_bop_tless_qualification | research | tools/summarize_bop_tless_qualification.py | Aggregate three-seed BOP T-LESS bbox and task-native baseline/trained evidence and compare an independent rerun semantically. |
...
| train_detectron2_runtime_smoke | experimental | tools/train_detectron2_runtime_smoke.py | Run bounded real Detectron2 training on a YOLOZU COCO wrapper and emit hash/resource evidence. |
