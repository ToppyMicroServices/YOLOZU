...
| generate_web_docs | stable | tools/generate_web_docs.py | Generate searchable onboarding web docs from the tool manifest, JSON Schemas, and curated repository sources. |
...
| run_ttt_compare | research | tools/run_ttt_compare.py | Run a fail-closed baseline-vs-adapted TTT local diagnostic with full checkpoint preflight, atomic stage status, and non-COCO proxy naming. |
