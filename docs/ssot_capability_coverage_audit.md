...
| Environment-qualified adaptive local vision | Profiling and registry-loading foundation only; target routing maturity is Experimental | Strict request, environment, bundle, lifecycle, evidence, eligibility-observation, and SelectionDecision validators exist. The live profiler uses bounded code-owned probes. The bundle loader validates the exact packaged SSOT or an operator-asserted workspace catalog with deterministic ordering and no model-runtime import. No router, model adapter, or adaptive execution surface is claimed. | `yolozu doctor` additively returns `environment_profile`; `load_algorithm_bundle_registry` is an integration API, not a recommendation command | The empty registry/lifecycle SSOT, roadmap, adaptive schemas, and tool manifest are packaged; source and packaged manifest copies match | `doctor_diagnostics.md`, `schema_governance.md`, `adaptive_image_routing.md`, `roadmap.md`, `production_readiness.md`, `reports/adaptive_vision_roadmap.md` | `tests/test_adaptive_environment_profile.py` covers live probes; `tests/test_adaptive_bundle_registry.py` covers packaged/custom trust, parser limits, ordering, path confinement, lifecycle failures, and import safety | Beads `YOLOZU-ll2.81` is the live planning source. Failed probes remain unknown; custom catalogs are nonselectable; profiles and checksum integrity are not qualification evidence; the Stable predictions lane is unchanged. |
