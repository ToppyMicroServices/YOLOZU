...
BOP object 6DoF readiness update: 2026-07-28
SDFT-style continual qualification update: 2026-07-28
...
| BOP T-LESS object 6DoF | Research | `tools/download_bop_dataset.py`, `tools/prepare_bop_yolozu.py`, `tools/eval_pose.py`, RunPod scripts | Safe downloader/converter plus a diagnostic train/export/eval frame holdout | `download_bop_dataset` and `prepare_bop_yolozu` are Research entries with source/packaged contracts and schemas | `bop_tless_protocol.md`, `reports/bop_pose_readiness_2026-07-28.md`, `manual/chapters/16_depth_6dof_symmetry.tex` | `tests/test_bop_pose_pipeline_safety.py` covers archive traversal, output ownership/source overlap, partitions, CAD scaling/hashes, sidecars, and shell help | Wiring/safety is locally qualified. A full real baseline-versus-trained three-seed result with ADD/ADDS, runtime/cost, release archive, and independent reproduction remains open. This is rigid-object pose; human 3D skeleton pose is unsupported. |
...
| Training platform: external and real-image fine-tuning | Stable YOLOX interface sub-lane; other lanes Experimental; execution environment-qualified | `yolozu/training/platform.py`, `tools/support_external_training.py`, `tools/run_external_finetune_smoke.py`, `tools/run_real_multitask_finetune_demo.py`, `tools/qualify_finetune_lanes.py` | `qualify_finetune_lanes` runs the combined matrix; `yolozu train --external-backend ...` and helpers remain separately callable | Experimental qualification entry and `finetune_lane_qualification_json` schema; source and packaged manifests match | `training_capability_matrix.md`, `training_orchestration.md`, `external_finetune_smoke.md`, `training_inference_export.md`, `reports/finetune_lane_evidence_2026-07-29.md` | external/real runner tests plus a clean-source schema-defined non-dry qualification with 111 verified artifact hashes | Non-dry projection-only lanes fail closed, and missing executables are machine-readable. The five-stage run completed with matching checkpoint handoffs, but every after bbox mAP was 0.0, only bbox validation is task-native, and keypoint/depth/pose6d labels are heuristic. Only RT-DETR executed in the five-framework external matrix. Training quality remains `not_established`; decision `hold`; maturity Experimental. |
| Continual learning / self-distillation | Research | `rtdetr_pose/tools/train_continual.py`, `tools/eval_continual.py`, `tools/qualify_sdft_continual.py`, continual regularizers | `qualify_sdft_continual` runs the fixed sequence; train/eval/decision helpers remain separately callable | Research entry and `sdft_continual_qualification_json` schema; source and packaged manifests match | `research_lanes.md`, `continual_learning.md`, `distillation.md`, `reports/sdft_continual_evidence_2026-07-28.md` | `tests/test_continual_metrics.py`, `tests/test_qualify_sdft_continual_cli.py`; clean three-seed real-COCO bundle with 434 verified file hashes | The prior raw pre-task score is no longer mislabeled as FWT. The tracked path records a real COCO matrix, initial baseline, checkpoint/teacher/data-order hashes, time, memory, and fairness checks. The release-addressable bounded run returned zero task metrics and zero naive-versus-SDFT-style deltas, so the decision remains `hold`, efficacy is `not_established`, and independent reproduction remains open. |
| TTA and TTT | TTA is Experimental; TTT is Research | `yolozu/tta/`, `tools/export_predictions.py`, `tools/run_ttt_evidence_suite.py` | Opt-in `--tta` has default postprocess and `rtdetr_pose` model-branch modes; opt-in `--ttt` updates parameters; the evidence suite separates sample-reset and continual-stream matrices | `export_predictions` is stable at entrypoint level; optional acceleration flags require qualification, while TTA and TTT retain narrower maturity | `tta_support_matrix.md`, `training_inference_export.md`, `ttt_protocol.md`, `research_lanes.md` | `tests/test_tta.py`, `tests/test_export_predictions_ttt_cli.py`, `tests/test_run_ttt_compare_tool.py`, `tests/test_run_ttt_evidence_suite.py`, `reports/ttt_evidence_2026-07-27.md` | Default TTA transforms predictions; `rtdetr_pose` model mode reruns one augmented branch; neither TTA mode updates parameters. TTT remains Research and the 2026-07-27 diagnostic does not establish efficacy. |
...
- The generated web-docs bundle is current for 122 tools and 27 JSON Schemas.
