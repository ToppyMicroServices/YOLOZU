...
| Environment-qualified adaptive local vision | Qualification foundation implemented; target routing maturity is Experimental | Strict typed validators, live environment profiling, trusted registry loading, pinned input/artifact readers, the frozen qualification protocol, child-process cancellation, and bounded managed publication exist. The qualifier retains exact repeat coverage and aggregate timing only. No router, recommendation service, registered model runner, or adaptive execution surface is claimed. | `yolozu doctor` returns `environment_profile`; `yolozu qualify-image-pipeline` is Experimental and currently stops at the empty packaged registry instead of creating dummy evidence | Empty registry/lifecycle/evidence SSOT, schemas, roadmap, and synchronized manifests are packaged | `doctor_diagnostics.md`, `schema_governance.md`, `adaptive_image_routing.md`, `roadmap.md`, `production_readiness.md`, `reports/adaptive_qualification_foundation_2026-08-25.md`, `reports/adaptive_vision_roadmap.md` | `tests/test_adaptive_qualification.py` injects clocks/runners and covers exact schedules, nearest rank, smoke status, aggregate report validation, and empty-registry refusal; existing environment, registry, and managed-output suites remain required | Beads `YOLOZU-ll2.81` is the live planning source. Fixture timing is not performance evidence. Full memory coverage stays unknown when unavailable. No real bundle qualification, model support, or human adoption is claimed. |
...
- The generated web-docs bundle is current for 132 tools and 46 JSON Schemas.
