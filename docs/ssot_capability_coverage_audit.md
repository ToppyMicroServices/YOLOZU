...
| Environment-qualified adaptive local vision | Experimental MCP recommendation and pinned processing; candidate-screening, qualification, reviewed activation, dormant support-profile review, reviewed lifecycle maintenance/rollback, pure-selection, monitored-source, and candidate-isolation decision foundations implemented | Strict typed validators, live environment profiling, trusted registry loading, non-executing screening, pinned I/O, bounded qualification/publication, reviewed evidence activation, dormant support review, dry-run-first lifecycle maintenance, pure selection, recommendation, pinned processing, and a bounded official-source scout exist. Candidate isolation is explicitly `none_supported`; its probe observes only fixed backend presence and cannot execute or promote a backend. Lifecycle updates require exact immutable identities and observed heads. Rollback restores only the complete historical set to a previously assigned, currently eligible same-family target or explicit `none`; a never-assigned Candidate remains in the separate promotion path, and global revoke is terminal. Three model-zoo entries remain non-promoted Candidate metadata with unbound execution. No registered adaptive model runner, model adapter, automatic rollback, or real execution claim is made. | `probe_candidate_isolation` reports fail-closed JSON; `yolozu scout-algorithms` collects only with `--collect`; qualification emits an unactivated report; evidence, support, and lifecycle mutation require exact review gates plus `--approve`; `update-image-pipeline-lifecycle` never mutates from metrics alone; MCP recommendation abstains by default; MCP processing defaults to no-write dry-run | Candidate-only registry/lifecycle SSOT, empty screening/support-profile/evidence streams, candidate-isolation probe schema, lifecycle rollback bindings and support-profile schemas, synchronized manifests, and generated references are packaged | `candidate_isolation_threat_model.md`, `algorithm_intake/README.md`, `adaptive_image_routing.md`, `schema_governance.md`, `production_readiness.md`, `reports/adaptive_lifecycle_rollback_foundation_2026-08-26.md`, `reports/adaptive_support_profile_governance_2026-08-26.md`, `reports/adaptive_candidate_screening_foundation_2026-08-26.md`, `reports/adaptive_baseline_bundle_registry_2026-08-26.md`, `reports/adaptive_routing_installed_verification_2026-08-26.md`, `reports/adaptive_vision_roadmap.md` | `tests/test_candidate_isolation_policy.py` proves that present executables and fixtures remain unsupported; lifecycle, support, routing, and installed tests retain the dormant, abstention, and package boundaries | Beads `YOLOZU-ll2.81` is the live planning source. Fixtures are interface tests, not containment or performance evidence. No canonical lifecycle event was appended. The three records remain Candidate; isolated acquisition/runner tasks stay deferred; screening, support-profile, and evidence streams and runner maps remain empty; default recommendation abstains with `maturity_disallowed`. No real bundle qualification, support, execution, or human adoption is claimed. |
...
- The generated web-docs bundle is current for 139 tools and 52 JSON Schemas.
