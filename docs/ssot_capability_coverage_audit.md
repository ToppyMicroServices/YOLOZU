...
| Training platform: external and real-image fine-tuning | Stable YOLOX interface sub-lane; other lanes Experimental; execution environment-qualified | `yolozu/training/platform.py`, `tools/support_external_training.py`, `tools/run_external_finetune_smoke.py`, `tools/run_real_multitask_finetune_demo.py`, `tools/qualify_finetune_lanes.py` | `qualify_finetune_lanes` runs the combined matrix; `yolozu train --external-backend ...` and helpers remain separately callable | Experimental qualification entry and `finetune_lane_qualification_json` schema; source and packaged manifests match | `training_capability_matrix.md`, `training_orchestration.md`, `external_finetune_smoke.md`, `training_inference_export.md` | external/real runner tests plus schema-defined non-dry qualification | Non-dry projection-only lanes now fail closed. The tracked five-stage run records hashes, handoff compatibility, runtime/memory and label provenance, but only bbox validation is task-native; keypoint/depth/pose6d labels are heuristic. External runtime absence is machine-readable. Training quality remains `not_established` and maturity is retained. |
