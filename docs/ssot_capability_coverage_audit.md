...
| Environment-qualified adaptive local vision | Experimental MCP recommendation and pinned processing; candidate-screening, qualification, reviewed activation, dormant support-profile review, reviewed lifecycle maintenance/rollback/promotion, pure-selection, monitored-source, and candidate-isolation decision foundations implemented | Strict typed validators, live environment profiling, trusted registry loading, non-executing screening, pinned I/O, bounded qualification/publication, reviewed evidence activation, dormant support review, dry-run-first lifecycle maintenance and promotion, pure selection, recommendation, pinned processing, and a bounded official-source scout exist. Candidate isolation is explicitly `none_supported`; its probe observes only fixed backend presence and cannot execute or promote a backend. Promotion separates Candidate-to-Experimental from Experimental-to-Stable, requires the complete exact managed profile/evidence set and human review, and adds first/later Stable absolute/comparator gates. Rollback restores only the complete historical set to a previously assigned, currently eligible same-family target or explicit `none`; global revoke is terminal. Three model-zoo entries remain non-promoted Candidate metadata with unbound execution. No registered adaptive model runner, model adapter, automatic promotion/rollback, or real execution claim is made. | `probe_candidate_isolation` reports fail-closed JSON; `yolozu scout-algorithms` collects only with `--collect`; qualification emits an unactivated report; evidence, support, lifecycle, and promotion mutation require exact review gates plus `--approve`; `promote-image-pipeline` rejects site evidence and Stable profile-set/regression drift; MCP recommendation abstains by default; MCP processing defaults to no-write dry-run | Candidate-only registry/lifecycle SSOT, empty screening/support-profile/evidence streams, candidate-isolation probe schema, lifecycle/promotion metadata and support-profile schemas, synchronized manifests, and generated references are packaged | `candidate_isolation_threat_model.md`, `algorithm_intake/README.md`, `adaptive_image_routing.md`, `schema_governance.md`, `production_readiness.md`, `reports/adaptive_promotion_governance_2026-08-29.md`, `reports/adaptive_promotion_failure_drills_2026-08-29.md`, `reports/adaptive_lifecycle_rollback_foundation_2026-08-26.md`, `reports/adaptive_support_profile_governance_2026-08-26.md`, `reports/adaptive_candidate_screening_foundation_2026-08-26.md`, `reports/adaptive_baseline_bundle_registry_2026-08-26.md`, `reports/adaptive_routing_installed_verification_2026-08-26.md`, `reports/adaptive_vision_roadmap.md` | Promotion, lifecycle, support, routing, candidate-isolation, and installed tests retain exact managed evidence, dry-run, atomic append, dormant, abstention, and package boundaries. Six checksum-bound offline failure fixtures cover invalid artifact, runtime, timeout, metric, license, and interface paths plus exact rollback. | Beads `YOLOZU-ll2.81` is the live planning source. Fixtures are interface tests, not containment or performance evidence. No canonical promotion or maintenance event was appended. The three records remain Candidate; isolated acquisition/runner tasks stay deferred; screening, support-profile, and evidence streams and runner maps remain empty; default recommendation abstains with `maturity_disallowed`. No real bundle qualification, support, execution, or human adoption is claimed. |
...
- The generated web-docs bundle is current for 140 tools and 52 JSON Schemas.
