...
| Environment-qualified adaptive local vision | Qualification, reviewed activation, and pure-selection foundations implemented; target routing maturity is Experimental | Strict typed validators, live environment profiling, trusted registry loading, pinned input/artifact readers, the frozen qualification protocol, child-process cancellation, bounded managed publication, reviewed append-only activation transitions, and a file-free deterministic selector exist. The selector applies the fixed channel, trust, evidence, hard-gate, and ranking order only to validated in-memory observations. No CLI/MCP recommendation service, registered model runner, or adaptive execution surface is claimed. | `yolozu doctor` returns `environment_profile`; `yolozu qualify-image-pipeline` emits an unactivated report; `yolozu activate-qualification-evidence` dry-runs by default and requires explicit review plus `--approve` for one atomic transition; the selector is Python-internal only | Empty registry/lifecycle/evidence SSOT, schemas, roadmap, and synchronized manifests are packaged | `doctor_diagnostics.md`, `schema_governance.md`, `adaptive_image_routing.md`, `roadmap.md`, `production_readiness.md`, `reports/adaptive_qualification_foundation_2026-08-25.md`, `reports/adaptive_vision_roadmap.md` | `tests/test_adaptive_qualification.py` covers fixed collection semantics and empty-registry refusal; `tests/test_adaptive_evidence_activation.py` covers reviewed state transitions; `tests/test_adaptive_selector.py` covers no-I/O filtering, channel collapse, trust, evidence, site scope, isolation, hard gates, and exact ranking | Beads `YOLOZU-ll2.81` is the live planning source. Fixture timing is not performance evidence. The registry and public evidence stream remain empty. No real bundle qualification, model support, or human adoption is claimed. |
...
- The generated web-docs bundle is current for 133 tools and 46 JSON Schemas.
