...
| Environment-qualified adaptive local vision | Qualification and reviewed activation foundations implemented; target routing maturity is Experimental | Strict typed validators, live environment profiling, trusted registry loading, pinned input/artifact readers, the frozen qualification protocol, child-process cancellation, bounded managed publication, and reviewed append-only activation transitions exist. The qualifier retains exact repeat coverage and aggregate timing only. No router, recommendation service, registered model runner, or adaptive execution surface is claimed. | `yolozu doctor` returns `environment_profile`; `yolozu qualify-image-pipeline` emits an unactivated report; `yolozu activate-qualification-evidence` dry-runs by default and requires explicit review plus `--approve` for one atomic transition | Empty registry/lifecycle/evidence SSOT, schemas, roadmap, and synchronized manifests are packaged | `doctor_diagnostics.md`, `schema_governance.md`, `adaptive_image_routing.md`, `roadmap.md`, `production_readiness.md`, `reports/adaptive_qualification_foundation_2026-08-25.md`, `reports/adaptive_vision_roadmap.md` | `tests/test_adaptive_qualification.py` covers fixed collection semantics and empty-registry refusal; `tests/test_adaptive_evidence_activation.py` covers dry-run, activation, supersession, terminal revoke, stale heads, trust boundaries, blocked lifecycle, personal actors, and interrupted writes | Beads `YOLOZU-ll2.81` is the live planning source. Fixture timing is not performance evidence. The registry and public evidence stream remain empty. No real bundle qualification, model support, or human adoption is claimed. |
