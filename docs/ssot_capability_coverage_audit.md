...
| Environment-qualified adaptive local vision | Future delivery; target Experimental | No router, new model adapter, or adaptive execution surface is claimed by this projection | `generate_adaptive_vision_roadmap` only renders/checks documentation; it does not perform recommendation or inference | `adaptive_vision_roadmap.json` and its schema are packaged; the generator is manifest-declared and the source/packaged tool manifests match | `roadmap.md`, `production_readiness.md`, `reports/adaptive_vision_roadmap.md` | `tests/test_adaptive_vision_roadmap_generator.py`; CI, pre-push, release, wheel, and sdist drift/presence gates | Beads `YOLOZU-ll2.81` is the live planning source. The JSON is a dated public scope projection, the Markdown is not qualification evidence, and the current Stable predictions lane is unchanged. |
