...
| Environment-qualified adaptive local vision | Future delivery; target Experimental | Strict request, bundle, lifecycle, evidence, eligibility-observation, and SelectionDecision validators exist; no router, model adapter, or adaptive execution surface is claimed | `generate_adaptive_vision_roadmap` only renders/checks documentation; it does not perform recommendation or inference | The roadmap, adaptive schemas, and tool manifest are packaged with byte-identical source/packaged manifest copies | `adaptive_image_routing.md`, `roadmap.md`, `production_readiness.md`, `reports/adaptive_vision_roadmap.md` | Focused adaptive interface-contract tests plus CI, pre-push, release, fresh-wheel, wheel, and sdist resource gates | Beads `YOLOZU-ll2.81` is the live planning source. The JSON is a dated public scope projection, the Markdown is not qualification evidence, and the current Stable predictions lane is unchanged. |
