...
| Environment-qualified adaptive local vision | Experimental read-only MCP recommendation; qualification, reviewed activation, and pure-selection foundations implemented | Strict typed validators, live environment profiling, trusted registry loading, pinned input/artifact readers, the frozen qualification protocol, child-process cancellation, bounded managed publication, reviewed append-only activation transitions, a file-free deterministic selector, and bounded read-only recommendation orchestration exist. The MCP service checks non-I/O gates before candidate artifact access and returns a complete selected or abstained SelectionDecision. No registered model runner, model adapter, or adaptive execution surface is claimed. | `yolozu doctor` returns `environment_profile`; `yolozu qualify-image-pipeline` emits an unactivated report; `yolozu activate-qualification-evidence` dry-runs by default and requires explicit review plus `--approve`; MCP `recommend_image_pipeline` is read-only and defaults to abstention with the empty packaged registry | Empty registry/lifecycle/evidence SSOT, schemas, roadmap, synchronized manifests, and generated 26-tool MCP reference are packaged | `doctor_diagnostics.md`, `schema_governance.md`, `adaptive_image_routing.md`, `ai_first.md`, `roadmap.md`, `production_readiness.md`, `reports/adaptive_qualification_foundation_2026-08-25.md`, `reports/adaptive_vision_roadmap.md` | `tests/test_adaptive_recommendation.py` covers empty-registry abstention, path confinement, malformed input, privacy, and no-write behavior; `tests/test_adaptive_selector.py` covers fixed filtering and ranking; live MCP/reference tests cover schema and surface parity | Beads `YOLOZU-ll2.81` is the live planning source. Fixture timing is not performance evidence. The registry and public evidence stream remain empty. No real bundle qualification, model support, execution, or human adoption is claimed. |
...
- The generated web-docs bundle is current for 134 tools and 46 JSON Schemas.
