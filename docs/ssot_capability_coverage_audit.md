...
| Dataset I/O and mask-only label derivation | Explicitly deferred as standalone capabilities | `yolozu/datasets/dataset.py`, `dataset_contract.py`, `dataset_validator.py` | `yolozu validate dataset`, `doctor train-dataset`, `export-dataset`, `import` | Covered by stable `yolozu` and dataset preparation entries; no separate mask-derivation command | `yolozu_spec.md`, `dataset_contract.md`, `training_inference_export.md`, `reports/dataset_preflight_2026-07-27.md` | `tests/test_dataset_contract.py`, `tests/test_dataset_mask_labels.py`, `tests/test_dataset_validator.py`, `tests/test_doctor_import_cli.py`, `data/smoke/`, `data/coco128/` | Empty splits fail closed; doctor direct-readiness and validation share strict readable-image/bbox checks; the recorded `1e-6` tolerance applies only to derived bbox-edge round-off and never clips labels. Explicit COCO `--instances`/`--images-dir` preflight is covered. Standalone maturity remains deferred. |
...
| Installed CLI and mixed-lane entrypoints | Mixed; maturity is per entrypoint, with narrower sub-lane rules | `yolozu/cli.py`, `cli_entry.py`, `cli_commands.py` | 28 canonical commands/aliases in current top-level help | 117 entries: 59 stable, 44 experimental, 14 research; source and packaged copies match | `generated/cli_reference.md`, `tools_index.md`, `manifest_declarative_spec.md` | 108 Python entrypoints scanned with zero help/manifest flag gaps; manual audit passes | Stable parent maturity is explicitly non-transitive; generated reference and manifest descriptions repeat that boundary. |

## Confirmed checks

- `tools/manifest.json` and `yolozu/data/manifest/tools_manifest.json` are byte-identical.
- Strict manifest validation passes for all 117 entries.
- Per-entrypoint help audit scans 108 Python tools with zero execution errors and zero missing flags.
...
- The generated web-docs bundle is current for 117 tools and 22 JSON Schemas.
