...
| Environment-qualified adaptive local vision | Live profiling foundation only; target routing maturity is Experimental | Strict request, environment, bundle, lifecycle, evidence, eligibility-observation, and SelectionDecision validators exist. The bounded live profiler runs code-owned probes and optional imports in isolated process groups. No router, model adapter, or adaptive execution surface is claimed. | `yolozu doctor` additively returns `environment_profile`; roadmap generation does not perform recommendation or inference | The roadmap, adaptive schemas, and tool manifest are packaged with byte-identical source/packaged manifest copies | `doctor_diagnostics.md`, `adaptive_image_routing.md`, `roadmap.md`, `production_readiness.md`, `reports/adaptive_vision_roadmap.md` | `tests/test_adaptive_environment_profile.py` covers CPU-only, Apple Silicon, NVIDIA, unsupported OS, probe limits, process-tree timeout, privacy, and installed doctor compatibility | Beads `YOLOZU-ll2.81` is the live planning source. Failed probes remain unknown, EnvironmentProfile is not qualification evidence, and the current Stable predictions lane is unchanged. |
