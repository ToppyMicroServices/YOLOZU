...
Labeled sample and compatibility update: 2026-09-10
...
| Environment-qualified adaptive local vision | Experimental MCP recommendation and pinned processing; candidate-screening, qualification, reviewed activation, dormant support-profile review, reviewed lifecycle maintenance/rollback/promotion, pure-selection, monitored-source, freshness-monitoring, and candidate-isolation decision foundations implemented | Strict typed validators, live environment profiling, trusted registry loading, non-executing screening, pinned I/O, bounded qualification/publication, reviewed evidence activation, dormant support review, dry-run-first lifecycle maintenance and promotion, pure selection, recommendation, pinned processing, a bounded official-source scout, and a read-only qualification freshness check exist. Candidate isolation is explicitly `none_supported`; its probe observes only fixed backend presence and cannot execute or promote a backend. Three model-zoo entries remain non-promoted Candidate metadata with unbound execution. No registered adaptive model runner, model adapter, automatic promotion/rollback, or real execution claim is made. | `probe_candidate_isolation` reports fail-closed JSON; `yolozu scout-algorithms` collects only with `--collect`; `yolozu check-qualification-freshness` warns without extending evidence; qualification emits an unactivated report; evidence, support, lifecycle, and promotion mutation require exact review gates plus `--approve`; MCP recommendation abstains by default; MCP processing defaults to no-write dry-run | Candidate-only registry/lifecycle SSOT, two current hold screening records, empty support-profile/evidence streams, candidate-isolation and freshness schemas, lifecycle/promotion metadata, synchronized manifests, and generated references are packaged | `candidate_isolation_threat_model.md`, `algorithm_intake/README.md`, `adaptive_candidate_screenings_2026-08-29.md`, `qualification_freshness.md`, `adaptive_image_routing.md`, `schema_governance.md`, `production_readiness.md`, `reports/adaptive_promotion_governance_2026-08-29.md`, `reports/adaptive_promotion_failure_drills_2026-08-29.md`, `reports/adaptive_lifecycle_rollback_foundation_2026-08-26.md`, `reports/adaptive_support_profile_governance_2026-08-26.md`, `reports/adaptive_candidate_screening_foundation_2026-08-26.md`, `reports/adaptive_baseline_bundle_registry_2026-08-26.md`, `reports/adaptive_routing_installed_verification_2026-08-26.md`, `reports/adaptive_vision_roadmap.md` | Promotion, lifecycle, support, routing, candidate-isolation, screening, freshness, and installed tests retain exact managed evidence, dry-run, atomic append, dormant, abstention, and package boundaries. Six checksum-bound offline failure fixtures cover invalid artifact, runtime, timeout, metric, license, and interface paths plus exact rollback. | Beads `YOLOZU-ll2.81` is the live planning source. Fixtures are interface tests, not containment or performance evidence. No canonical promotion or maintenance event was appended. The three registry records remain Candidate; the two screened candidates are current holds; isolated acquisition/runner tasks stay deferred; support-profile and evidence streams and runner maps remain empty; default recommendation abstains with `maturity_disallowed`. No real bundle qualification, support, execution, or human adoption is claimed. |
| Streaming and tracking interface contracts | Contract-only foundation; no runtime maturity claim | Standard-library validators define bounded per-frame stream results, aggregate summaries, exact drop/rate accounting, an unactivated qualification-report record shape, explicit selection or abstention, and session-scoped tracking transitions. | None | Eight canonical schemas and byte-identical packaged copies are registered in both manifests and release resources. | `streaming_interface_contract.md`, `tracking_interface_contract.md`, `schema_governance.md` | `tests/test_streaming_contract.py`, `tests/test_tracking_contract.py`, schema-governance and installed-artifact tests | The transport and semantic boundaries are implemented. No decoder, live schedule result, tracker, model, runner, qualification evidence, availability, or support claim is included. |
...
- The generated web-docs bundle is current for 142 tools and 63 JSON Schemas.
