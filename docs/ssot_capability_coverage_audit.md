...
| BOP T-LESS object 6DoF | Research | `tools/download_bop_dataset.py`, `tools/prepare_bop_yolozu.py`, `tools/eval_pose.py`, RunPod scripts | Safe downloader/converter plus a diagnostic train/export/eval frame holdout | Research entries with source/packaged contracts and BOP conversion/qualification schemas | `bop_tless_protocol.md`, `reports/bop_tless_evidence_2026-07-30.md`, `manual/chapters/16_depth_6dof_symmetry.tex` | Safety tests plus real base/models/train_primesense hashes, strict GT counts, three seeds, runtime/memory/checkpoint hashes, and independent semantic reproduction | Real acquisition, strict task-native before/after fields, and independent reproduction are complete. No prediction matched pose GT, so the result is `hold`/`not_established`. The frame holdout is not the official BOP test. This is rigid-object pose; human 3D skeleton pose is unsupported. |
...
| Training platform: external and real-image fine-tuning | Stable YOLOX interface sub-lane; other lanes Experimental; execution environment-qualified | `yolozu/training/platform.py`, `tools/support_external_training.py`, `tools/run_external_finetune_smoke.py`, bounded HF/Detectron2 launchers, qualification tools | `qualify_finetune_lanes` and backend helpers remain separately callable | Experimental qualification entries and schemas; source and packaged manifests match | `training_capability_matrix.md`, `external_finetune_smoke.md`, `reports/external_runtime_evidence_2026-07-30.md` | Ultralytics, HF DETR, and Detectron2 real training/export/eval in two Python/Torch environments; five structured runtime failures | Config projection is not training, swallowed traceback/zero-exit is rejected, and resource/checkpoint/prediction hashes are recorded. Three lanes reproduced zero-mAP semantics; five were unavailable on the tested host. Training quality remains `not_established`; decision `hold`; maturity Experimental. |
| Continual learning / self-distillation | Research | `rtdetr_pose/tools/train_continual.py`, `tools/eval_continual.py`, `tools/qualify_sdft_continual.py`, continual regularizers | `qualify_sdft_continual` runs the fixed sequence; train/eval/decision helpers remain separately callable | Research entry and `sdft_continual_qualification_json` schema; source and packaged manifests match | `research_lanes.md`, `continual_learning.md`, `distillation.md`, `reports/sdft_continual_evidence_2026-07-28.md` | Clean three-seed real-COCO bundle with 434 verified hashes and a clean Python 3.12 rerun of all 12 train/eval paths | The release-addressable bounded run and its independent reproduction returned zero task metrics and zero naive-versus-SDFT-style deltas. Reproducibility is established for the diagnostic; decision remains `hold` and efficacy `not_established`. |
| TTA and TTT | TTA is Experimental; TTT is Research | `yolozu/tta/`, `tools/export_predictions.py`, `tools/run_ttt_evidence_suite.py` | Opt-in `--tta` has default postprocess and `rtdetr_pose` model-branch modes; opt-in `--ttt` updates parameters; the evidence suite separates sample-reset and continual-stream matrices | `export_predictions` is stable at entrypoint level; optional acceleration flags require qualification, while TTA and TTT retain narrower maturity | `tta_support_matrix.md`, `training_inference_export.md`, `ttt_protocol.md`, `research_lanes.md` | Tests plus the release bundle and a clean rerun of all 30 matrix cells with zero semantic differences | Default TTA transforms predictions; `rtdetr_pose` model mode reruns one augmented branch; neither TTA mode updates parameters. TTT diagnostic reproducibility is established, but zero improvement keeps efficacy `not_established` and maturity Research. |
...
- The generated web-docs bundle is current for 126 tools and 29 JSON Schemas.
