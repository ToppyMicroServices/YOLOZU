...
| Continual learning / self-distillation | Research | `rtdetr_pose/tools/train_continual.py`, `tools/eval_continual.py`, `tools/qualify_sdft_continual.py`, continual regularizers | `qualify_sdft_continual` runs the fixed sequence; train/eval/decision helpers remain separately callable | Research entry and `sdft_continual_qualification_json` schema; source and packaged manifests match | `research_lanes.md`, `continual_learning.md`, `distillation.md` | `tests/test_continual_metrics.py`, `tests/test_qualify_sdft_continual_cli.py`; three-seed real-COCO execution probe | The prior raw pre-task score is no longer mislabeled as FWT. The tracked path now records a real COCO matrix, initial baseline, checkpoint/teacher/data-order hashes, time, memory, and fairness checks. The bounded probe returned zero metrics for naive and SDFT-style runs, so efficacy remains `not_established` and release/independent evidence remains open. |
