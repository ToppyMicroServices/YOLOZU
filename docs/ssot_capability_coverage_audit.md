...
| Environment-qualified adaptive local vision | Profiling, registry-loading, and managed-output foundation only; target routing maturity is Experimental | Strict request, environment, bundle, lifecycle, evidence, eligibility-observation, and SelectionDecision validators exist. The live profiler uses bounded code-owned probes. The bundle loader validates the exact packaged SSOT or an operator-asserted workspace catalog with deterministic ordering and no model-runtime import. `ManagedOutputTransaction` adds POSIX directory-relative no-follow publication, exact manifest validation, bounded force cleanup, and known-state recovery. No router, model adapter, or adaptive execution surface is claimed. | `yolozu doctor` additively returns `environment_profile`; the registry loader and managed-output transaction are integration APIs, not recommendation or processing commands | The empty registry/lifecycle SSOT, roadmap, adaptive schemas, and tool manifest are packaged; source and packaged manifest copies match | `doctor_diagnostics.md`, `schema_governance.md`, `adaptive_image_routing.md`, `roadmap.md`, `production_readiness.md`, `reports/adaptive_vision_roadmap.md` | Environment and registry suites cover live probes and bounded trust; `tests/test_adaptive_managed_output.py` covers manifests, bounds, force, fault injection, identity swaps, recovery, and import safety | Beads `YOLOZU-ll2.81` is the live planning source. Failed probes remain unknown; custom catalogs are nonselectable; output publication is not image processing or qualification evidence; non-POSIX platforms fail closed; the Stable predictions lane is unchanged. |
...
- The generated web-docs bundle is current for 131 tools and 32 JSON Schemas.
