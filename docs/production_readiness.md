...
| Environment-qualified adaptive local vision | Experimental MCP recommendation and pinned processing; candidate screening, qualification, reviewed activation, dormant support-profile review, reviewed lifecycle maintenance/rollback, and pure-selection foundations implemented | `doctor` emits a privacy-safe EnvironmentProfile. The exact Torchvision Candidate uses a user-prepared local artifact, `NOASSERTION`, and a code-owned local COCO evaluator. Its retained 16-image report passed the fixed quality and throughput gates but returned `hold` after exceeding the preregistered p95 latency limit. The limit was not changed after measurement. No activation, support assignment, or promotion followed, so ordinary routing still cannot select Candidate and the default service abstains. | [`doctor_diagnostics.md`](doctor_diagnostics.md), [`adaptive_image_routing.md`](adaptive_image_routing.md), [`image_service_candidate_review.md`](image_service_candidate_review.md), [`../reports/adaptive_support_profile_governance_2026-08-26.md`](../reports/adaptive_support_profile_governance_2026-08-26.md), [`../reports/adaptive_qualification_foundation_2026-08-25.md`](../reports/adaptive_qualification_foundation_2026-08-25.md), [`../reports/adaptive_vision_roadmap.md`](../reports/adaptive_vision_roadmap.md), [`roadmap.md`](roadmap.md) |
...
components. The default packaged registry contains three unbound model-zoo
Candidates and one bound, qualification-only Torchvision Candidate whose full
report is `hold`; it therefore abstains with `maturity_disallowed`. The
...
bound only to the held Candidate, which does not pass the required governance
