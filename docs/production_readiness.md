...
| Environment-qualified adaptive local vision | Experimental MCP recommendation and pinned processing; candidate screening, qualification, reviewed activation, dormant support-profile review, reviewed lifecycle maintenance/rollback, and pure-selection foundations implemented | `doctor` emits a privacy-safe EnvironmentProfile. Non-executing candidate screening derives pass, hold, or reject from separate mechanical and human-review facts; the packaged stream has two current hold records and no pass, and custom input cannot satisfy managed trust. `qualify-image-pipeline` publishes an unactivated measured report. Evidence activation, dormant support review, and lifecycle maintenance are dry-run by default and mutate only after exact review gates plus `--approve`. Lifecycle rollback restores only one exact historical channel set to a previously assigned, currently eligible same-family target or explicit `none`; a never-assigned Candidate remains in the promotion path, and there is no automatic or metrics-driven mutation. Recommendation and execution use the lifecycle-pinned historical set. Three model-zoo records remain non-promoted Candidate baselines with unbound execution. A code-owned Torchvision runner exists, but no packaged bundle binds it or completes the license, qualification, support, evidence, and lifecycle gates. The default recommendation therefore abstains, no real bundle can run, and no support or performance evidence is claimed. | [`doctor_diagnostics.md`](doctor_diagnostics.md), [`adaptive_image_routing.md`](adaptive_image_routing.md), [`adaptive_candidate_screenings_2026-08-29.md`](adaptive_candidate_screenings_2026-08-29.md), [`../reports/adaptive_lifecycle_rollback_foundation_2026-08-26.md`](../reports/adaptive_lifecycle_rollback_foundation_2026-08-26.md), [`../reports/adaptive_support_profile_governance_2026-08-26.md`](../reports/adaptive_support_profile_governance_2026-08-26.md), [`../reports/adaptive_candidate_screening_foundation_2026-08-26.md`](../reports/adaptive_candidate_screening_foundation_2026-08-26.md), [`../reports/adaptive_baseline_bundle_registry_2026-08-26.md`](../reports/adaptive_baseline_bundle_registry_2026-08-26.md), [`../reports/adaptive_qualification_foundation_2026-08-25.md`](../reports/adaptive_qualification_foundation_2026-08-25.md), [`../reports/adaptive_routing_installed_verification_2026-08-26.md`](../reports/adaptive_routing_installed_verification_2026-08-26.md), [`../reports/adaptive_vision_roadmap.md`](../reports/adaptive_vision_roadmap.md), [`roadmap.md`](roadmap.md) |
...
only the packaged SSOT or operator-asserted workspace catalog. Screening trust is
derived from its path, and only a current repository-managed pass can clear that gate. A separate pure
...
components. The default packaged registry contains three Candidate baselines with
unbound execution and therefore abstains with `maturity_disallowed`. The
paired `process_images` service adds dry-run-by-default pinned revalidation and an
explicit bounded execution/publication gate. A code-owned Torchvision runner is
registered, but no packaged bundle binds it or passes the required governance
gates, so it cannot currently run a real model. The service presence is not model
availability or qualification evidence.
The separate `update-image-pipeline-lifecycle` interface contract is dry-run-first.
