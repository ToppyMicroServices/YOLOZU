...
- `yolozu benchmark` 提供了更接近 Ultralytics 的 benchmark 入口；在条件满足时可实际编排 `torch` / `onnx` / `engine`，并为这些 backend 比较生成真实 parity artifact；`torchscript` 目前以诚实的 synthetic/skip 语义纳入统一报告，并显式报告被跳过的 format
