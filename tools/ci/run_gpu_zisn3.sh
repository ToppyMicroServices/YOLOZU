...
if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
  cat <<'EOF'
Usage: bash tools/ci/run_gpu_zisn3.sh

Runs the YOLOZU ZISN-3 GPU validation stage with hash-locked Python dependencies.
EOF
  exit 0
fi
...
python3 tools/ci/install_with_hashes.py \
  ...
  --requirements requirements-locks/requirements-zisn3-extra.lock
