...
- `yolozu benchmark`（Ultralytics parity を意識した benchmark 入口。利用可能なら `torch` / `onnx` / `engine` を実行し、`torchscript` は現段階では honest な synthetic/skip semantics で受け付け、未対応 format は skipped を明示）
