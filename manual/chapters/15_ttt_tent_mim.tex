...
\begin{enumerate}
  ...
  \item \textbf{Warning summary}: was adaptation skipped or constrained for a safety reason?
\end{enumerate}
...
\begin{tikzpicture}[
  node distance=8mm,
  font=\small,
  box/.style={draw, rounded corners, align=center, inner sep=6pt, minimum width=24mm},
  arrow/.style={-{Latex[length=2mm]}, thick}
]
...
\draw[arrow] (base) -- node[above]{same images} (adapt);
\draw[arrow] (adapt) -- node[above]{weights updated in memory} (pred);
\draw[arrow] (pred) -- (cmp);
\end{tikzpicture}
}
...
\subsection{MIM}
\textbf{Principle.}
MIM uses masked reconstruction and optional entropy terms to adapt from unlabeled shifted inputs. Instead of only asking the model to become more confident, it also asks the model to reconstruct hidden structure from partially masked internal features.
...
\begin{itemize}
  ...
  \item In the repo-backed smoke example, \cmd{steps\_run=2}, \cmd{mean\_final\_loss=0.461853}, and \cmd{changed\_images=0 / 2}. That means the MIM path is really running even though this tiny subset does not change final detections.
\end{itemize}
