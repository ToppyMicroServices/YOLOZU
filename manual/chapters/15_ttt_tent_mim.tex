...
python3 -m yolozu predict-images \
...
\begin{itemize}
  \item \path{map50 0.00326797 -> 0.00392157}
  \item \path{map50_95 0.000326797 -> 0.000392157}
\end{itemize}
...
\path{map50} from \path{0.00326797} to \path{0.00392157} and
\path{map50_95} from \path{0.000326797} to \path{0.000392157}.
...
\begin{itemize}
  ...
  \item \cmd{mim_probe}: a fixed real probe that demonstrates an actual before/after metric delta on the bundled shifted ten-image probe
\end{itemize}
...
\begin{itemize}
  ...
  \item The historical smoke record reports \cmd{steps_run=2}, \cmd{mean_final_loss=0.461853}, and \cmd{changed_images=0 / 2}; it is not current full-checkpoint evidence.
  \item The fixed real probe proves operator-visible impact:
  \cmd{steps_run=10}, \cmd{mean_final_loss=0.0791513}, and
  \cmd{changed_images=10 / 10}. The metrics move from
  \cmd{map50=0.00326797} to \cmd{0.00392157} and from
  \cmd{map50_95=0.000326797} to \cmd{0.000392157}.
  \item When \cmd{pycocotools} is not installed in the current runtime, the compare falls back to \cmd{simple_map_proxy} instead of silently skipping evaluation.
\end{itemize}

\textbf{Concrete repo results.}
\begin{itemize}
  \item historical smoke record: \cmd{steps_run=2}, \cmd{mean_final_loss=0.461853}, \cmd{changed_images=0 / 2}
  \item fixed real probe compare: \cmd{steps_run=10}, \cmd{mean_final_loss=0.0791513}, \cmd{changed_images=10 / 10}
  \item fixed real probe metrics: \cmd{map50=0.00326797} to
  \cmd{0.00392157}; \cmd{map50_95=0.000326797} to \cmd{0.000392157}
\end{itemize}
...
On the tiny smoke fixture, it intentionally skips adaptation (\cmd{steps_run=0}, \cmd{eata_empty_selected_set}), which is conservative behavior.
