...
\subsection{MIM}
\textbf{Principle.}
MIM uses masked reconstruction and optional entropy terms to adapt from unlabeled shifted inputs. Instead of only asking the model to become more confident, it also asks the model to reconstruct hidden structure from partially masked internal features. In practice, the references to know are Masked Autoencoders (MAE) by He et al.\ \cite{he2022mae} and SimMIM by Xie et al.\ \cite{xie2022simmim}; \yolozu{} uses the same family of masked-image-modeling ideas as an inference-time auxiliary signal rather than as a standalone pretraining recipe.
...
\begin{itemize}
  ...
  \item This is why MIM is shown as an execution example inside Figure~\ref{fig:ttt-real-summary}, not as the main metric-win bar chart.
\end{itemize}

\textbf{Concrete repo result.}
The current checked-in MIM example is the repo-backed smoke fixture:
\cmd{steps\_run=2}, \cmd{mean\_final\_loss=0.461853}, \cmd{changed\_images=0 / 2}.
This is useful because it proves that the masked-reconstruction path is wired correctly.
At the same time, it also shows why operators should not overclaim from a tiny smoke subset.
