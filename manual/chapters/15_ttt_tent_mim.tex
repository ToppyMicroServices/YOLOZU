...
\begin{center}
\begin{tabular}{p{1.8cm} p{3.8cm} p{2.6cm} p{1.3cm} p{1.8cm} p{3.1cm}}
\toprule
\textbf{Method} & \textbf{Run dir} & \textbf{Real compare status} & \textbf{Steps} & \textbf{Mean final loss} & \textbf{Warning summary} \\
\midrule
Tent & \path{reports/ttt_compare/tent_smoke_cpu} & completed & 2 & 4.214431 & none \\
MIM & \path{reports/ttt_compare/mim_smoke_cpu} & completed & 2 & 0.461853 & repo-backed config \path{configs/examples/ttt_compare/rtdetr_pose_mim_compare.json} \\
CoTTA & \path{reports/ttt_compare/cotta_smoke_cpu} & completed & 1 & 4.243579 & none \\
EATA & \path{reports/ttt_compare/eata_smoke_cpu} & completed & 0 & null & \cmd{eata\_empty\_selected\_set} on both smoke images \\
SAR & \path{reports/ttt_compare/sar_smoke_cpu} & completed & 2 & 4.211009 & none \\
\bottomrule
\end{tabular}
\end{center}
...
For pose-heavy runs, switch to \cmd{--ttt-preset pose_mim}. The shipped
\cmd{mim} boilerplate already routes both baseline and adapted export commands
through \path{configs/examples/ttt_compare/rtdetr_pose_mim_compare.json}, so
the repo-shipped checkpoint can run a real compare without a long manual CLI.
...
\begin{itemize}
  ...
  \item \cmd{steps\_run=2}
  \item \cmd{mean\_final\_loss=0.461853}
  \item \cmd{changed\_images=0 / 2}
\end{itemize}
