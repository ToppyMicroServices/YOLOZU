...
\WorkflowOpening
{Run bounded test-time adaptation experiments and compare before/after artifacts under a fixed target shift.}
{Use this when the model faces domain shift and you want controlled Tent, MIM, CoTTA, EATA, or SAR comparisons.}
{A fixed target image set, baseline export settings, TTT preset or method knobs, seed/reset policy, and output report paths.}
{\cmd{bash scripts/ttt_compare.sh --help} for the compare route, or \cmd{yolozu export --ttt ...} with bounded TTT flags.}
{Baseline and adapted predictions, compare JSON/Markdown, TTT logs, and optional method-specific research reports.}
{Check changed image counts, final losses, guard warnings, reset policy, runtime cost, and before/after metrics on the same target set.}
{No real domain shift, broad update filters, missing reset policy, unbounded streaming adaptation, or mixing TTA with TTT claims.}
...
\section{TTT Quickstart (operator first)}
This is the shortest safe route through the chapter:
\begin{enumerate}
  \item freeze a small target subset,
  \item run one baseline export and one adapted export,
  \item open the before/after compare markdown,
  \item only then inspect raw TTT logs or method internals.
\end{enumerate}
...
Expected outputs:
\begin{itemize}
  \item \path{reports/ttt_compare/tent/baseline_predictions.json}
  \item \path{reports/ttt_compare/tent/tent_predictions.json}
  \item \path{reports/ttt_compare/tent/tent_before_after_compare.md}
  \item \path{reports/ttt_compare/tent/tent_ttt_log.json}
\end{itemize}

How to verify the run:
\begin{itemize}
  \item open the compare markdown first,
  \item check that \cmd{steps_run > 0},
  \item check \cmd{changed_images} and the before/after metric fields,
  \item confirm the same dataset split, checkpoint, seed, and reset policy were used.
\end{itemize}

Common failures:
\begin{itemize}
  \item the target subset has no real domain shift, so predictions do not change,
  \item the checkpoint path is missing or incompatible with the config,
  \item the update filter is too broad for a first run,
  \item stream reset is used without an explicit batch cap.
\end{itemize}

\section{Scope and support}
\yolozu{} supports controlled test-time training (TTT) for the \path{rtdetr_pose} adapter via
\path{tools/export_predictions.py} or the canonical \cmd{yolozu export} / \cmd{python3 -m yolozu export} surface.
...
\begin{itemize}
  ...
  \item \textbf{Default update scope}: start with safe presets (norm-only / adapter-safe). Keep CoTTA/EATA/SAR constrained to LoRA/Norm-safe subsets first.
  \item \textbf{Reset recommendation}: for controlled comparisons, prefer \cmd{--ttt-reset sample}. For streaming, use \cmd{--ttt-reset stream} only with an explicit batch cap.
\end{itemize}
...
(\path{corruption}, \path{severity}, \path{seed}, \path{images_sha256}).

\subsection{Multi-seed evidence suite}
After preparing a shifted dataset and fully compatible base and MIM
checkpoints, run the complete clean/shift matrix with:
\begin{lstlisting}[language=bash]
...
The suite requires at least three unique seeds, fails when any child comparison
fails, and separates sample-reset from continual-stream results. It generates
COCO AP, calibration/collapse status, update ratio, latency, memory, and actual
forward/backward/optimizer counts. The 2026-07-27 local diagnostic completed all
30 comparisons but found no AP50:95 improvement; efficacy remains
\texttt{not\_established}. Its checkpoint and child-artifact bundle is
release-addressable at
\url{https://github.com/ToppyMicroServices/YOLOZU/releases/tag/ttt-evidence-2026-07-27}
with archive SHA-256:
\begin{quote}
\small\cmd{bb200d0c0a36447f0b6ed262a56ee09bef44ded8f10c55673243080fe1054068}
\end{quote}
On 2026-07-30, a clean Python 3.12 environment verified all 261 embedded
hashes and reran all 30 method/domain/seed cells at the exact source commit.
The 11-field child-result comparison and all aggregate rows had zero semantic
differences.  Independent reproducibility is established for this bounded
diagnostic, but the unchanged zero-delta result does not establish efficacy.
...
\begin{itemize}
  ...
  \item \path{meta.export_settings.domain_shift_recipe} (\path{path}, \path{sha256})
\end{itemize}

\subsection{Evidence boundary figures: read this first}
The checked-in figures are generated atomically by
\path{tools/render_ttt_manual_figures.py}. Their source is the validated
\path{docs/assets/ttt_method_results_source.json}. The checked-in source is a
synthetic fixture with no measured efficacy fields and
\cmd{promotion\_eligible=false}; the figures therefore explain provenance and
workflow boundaries rather than claiming an improvement.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/ttt_method_results_summary.png}
  \caption{TTT method evidence boundary. The figure distinguishes runnable YOLOZU profiles, fidelity, and the current \texttt{not\_established} efficacy conclusion. It is not a measured result chart.}
...
\begin{figure}[htbp]
  ...
  \caption{Fail-closed compare flow. Dataset, config, and checkpoint are validated first; a successful checkpoint report uses the canonical value \texttt{status=full} and \texttt{load.loaded=true}. Reports remain local diagnostics.}
  \label{fig:ttt-compare-workflow}
\end{figure}

\subsection{Local diagnostic demo}
\yolozu{} provides a self-contained diagnostic that trains a tiny local
checkpoint, applies a deterministic shift, and compares TTT off/on:

\begin{lstlisting}[language=bash,caption={TTT local diagnostic with non-COCO proxy AP.}]
...
The report explicitly uses \cmd{proxy\_ap50} and
\cmd{proxy\_ap50\_95}, declares \cmd{metric\_semantics=non\_coco\_proxy},
\cmd{evidence\_kind=local\_diagnostic},
\cmd{promotion\_eligible=false}, and
\cmd{efficacy\_conclusion=not\_established}. A positive or negative delta is a
local observation only.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/ttt_probe_example_panel.png}
  \caption{Provenance-only example panel from the validated synthetic fixture. It illustrates which artifacts a measured bundle would have to hash-bind; it is not qualitative efficacy evidence.}
...
\begin{itemize}
  ...
  \item \cmd{detector_response_safe}: selected-foreground class/box consistency with no-object exclusion
...
\cmd{sar_safe} selects \cmd{lora_only}; it does not silently choose an adapter
rank. Supply one explicitly. The exporter validates this before model setup:
\begin{lstlisting}[language=bash]
python3 tools/export_predictions.py \
  --adapter rtdetr_pose --dataset data/smoke \
  --config rtdetr_pose/configs/base.json \
  --lora-r 4 --ttt --ttt-preset sar_safe \
...
For detection-native response TTT, \cmd{--ttt-response-min-selected} defaults
to 1.  Below the configured count the response objective abstains.  With no
auxiliary loss active, backward and optimizer execution are skipped and the
normalization buffers are restored.  The report records both per-step and
aggregate abstention.  A higher threshold is
an evaluation choice, not an established quality improvement.

\section{Method Catalog with concrete examples}
...
\begin{enumerate}
  ...
  \item \path{<method>_before_after_compare.md} for the compact summary,
  \item \path{<method>_ttt_log.json} for adaptation loss/runtime details,
...
directly and force
\cmd{--ttt-update-filter norm_only}. Provide a current-compatible checkpoint
unless the selected workflow documents a different fixture. If a team owns a dedicated
...
\subsection{Historical smoke compare snapshot}
Source commit
\path{72f0862f2487c7a23267820cc2dfc4818e46118b} recorded boilerplate runs against
...
The claimed checkpoint config was not committed, and the checkpoint is not
fully compatible with the current model. These rows are preserved only as
historical workflow records; they are neither current execution evidence nor a
quality claim. Pinned hashes and the compatibility summary are in
\path{reports/rtdetr_pose_coco128_gpu_matcher_historical.json}.
...
\bottomrule
\end{tabular}
\caption{Historical smoke compare snapshot from source commit \texttt{72f0862} on \texttt{data/smoke}, \cmd{split=val}, \cmd{max-images=2}, \cmd{device=cpu}, with \cmd{--skip-eval}. This table preserves the recorded workflow outcome and is not current full-checkpoint evidence or a quality leaderboard.}
\end{table}
...
The smoke table above is a \textbf{historical workflow record}. It tells you
what the source commit recorded; it does not prove current execution,
reproducibility, or that one method is globally better than another.
...
\begin{itemize}
  \item treating \cmd{mean_final_loss} as a quality score across methods,
...
\cmd{eata_empty_selected_set}. That means the method decided \emph{not} to adapt on those smoke images, which is conservative behavior, not a crash.
...
\begin{tikzpicture}[
  ...
  note/.style={align=center, font=\scriptsize, fill=white, inner sep=2pt},
  arrow/.style={-{Latex[length=2mm]}, thick}
]
...
\node[note] (sameimg) at ($(base.north east)!0.5!(adapt.north west)+(0,7mm)$) {same images};
\node[note] (weights) at ($(adapt.north east)!0.5!(pred.north west)+(0,7mm)$) {weights updated in memory};
\begin{pgfonlayer}{bg}
...
\begin{enumerate}
  ...
  \item open \path{<method>_before_after_compare.md},
  \item only then inspect \path{<method>_ttt_log.json}.
\end{enumerate}
...
\begin{itemize}
  \item Open \path{tent_before_after_compare.md} first.
  \item Check whether \cmd{steps_run > 0}.
  \item If \cmd{changed_images=0}, adaptation completed but exported predictions did not change on this subset.
  \item Open \path{tent_ttt_log.json} only if you need guard or runtime detail.
\end{itemize}

\textbf{Concrete repo result.}
The detector-adapted Tent profile executes and records loss, update norms,
guard stops, rollback, and prediction deltas. Its profile is
\cmd{yolozu\_detector\_entropy\_v2}; it is not a reference-faithful Tent
reproduction, and efficacy is not established.
...
For pose-heavy runs, switch to \cmd{--ttt-preset pose\_mim}. \yolozu{} ships
two MIM boilerplate names:
\begin{itemize}
  \item \cmd{mim}: selects the tracked structured-MIM config
  \item \cmd{mim\_probe}: retains a YOLO26 MIM configuration alias, but does not bundle the historical ignored dataset or checkpoint
\end{itemize}
...
\textbf{How to read the result.}
\begin{itemize}
  \item Open \path{mim_before_after_compare.md} first.
  \item Then inspect \path{mim_ttt_log.json}; the MIM loss trace is more informative than Tent's entropy-only loss.
  \item Confirm checkpoint preflight reports \cmd{status=full} and \cmd{load.loaded=true}.
  \item When \cmd{pycocotools} is unavailable, the fallback is named
  \cmd{proxy_ap50}/\cmd{proxy_ap50_95}; it is not COCO mAP.
\end{itemize}

\textbf{Concrete repo result.}
Structured-hook validation, mask controls, loss accounting, and fail-closed
checkpoint preflight are implemented. No current checked-in measured bundle
...
\textbf{How to read the result.}
\begin{itemize}
  \item Open \path{cotta_before_after_compare.md} first.
  \item Then inspect \path{cotta_ttt_log.json} to confirm stream updates and runtime.
  \item Use \path{cotta_drift.md} when you need to reason about long-run stability.
\end{itemize}

\textbf{Concrete repo result.}
The YOLOZU phase-1 CoTTA variant executes and reports EMA, augmentation,
restoration, update-norm, and drift diagnostics. It is not reference-faithful
CoTTA, and efficacy is not established.
...
\textbf{How to read the result.}
\begin{itemize}
  \item Open \path{eata_before_after_compare.md} first.
  \item If \cmd{steps_run=0}, check the warning field before assuming anything failed.
  \item On tiny smoke subsets, \cmd{eata_empty_selected_set} is expected conservative behavior.
\end{itemize}

\textbf{Concrete repo result.}
The EATA selection and regularization path is implemented and covered by
diagnostic tests. The checked-in figure is a synthetic provenance fixture; no
current checked-in measured bundle establishes EATA efficacy.
...
\textbf{How to read the result.}
\begin{itemize}
  \item Open \path{sar_before_after_compare.md} first.
  \item Compare runtime in \path{sar_ttt_log.json} against Tent to understand the extra adaptation cost.
  \item Use \path{sar_robustness.md} when comparing SAR against CoTTA and EATA.
\end{itemize}

\textbf{Concrete repo result.}
The perturb/restore path and its guard diagnostics are implemented. The
checked-in figure is a synthetic provenance fixture; no current checked-in
measured bundle establishes SAR efficacy.
...
\textbf{Rule of thumb.}
Start from the task-aware Tent preset, then escalate to MIM/\allowbreak CoTTA/\allowbreak EATA/\allowbreak SAR only when the shift evidence justifies the extra complexity and latency.

\section{Implementation Notes: YOLOZU rollout priority (high \texorpdfstring{$\to$}{->} low)}
...
This section defines a safety-bounded phase-1 Research scope for EATA on detection
...
This section defines a safety-bounded phase-1 Research rollout scope for SAR in
\yolozu{}. It does not establish production readiness.
...
Evaluate TTT as a before/after artifact comparison, not as a live-model update.
The minimum acceptance check is:
\begin{itemize}
  \item baseline and adapted predictions use the same target subset,
  \item the report records method, preset, seed, reset policy, and update scope,
  \item runtime and warning fields are inspected before claiming an improvement,
  \item unchanged predictions are reported as such instead of treated as a failure.
\end{itemize}

Failure modes to check first:
\begin{itemize}
  \item \textbf{No domain shift}: the method runs but has no useful signal.
  \item \textbf{Unsafe update scope}: too many parameters move for a first comparison.
  \item \textbf{State leakage}: stream adaptation carries state into the next comparison.
  \item \textbf{Cost drift}: adaptation latency exceeds the budget for the target workflow.
\end{itemize}
...
for \path{tools/export_predictions.py} and the top-level \cmd{yolozu} CLI; \path{tools/yolozu.py} remains the repo-checkout compatibility wrapper.
...
For detector-native foreground response consistency, use the concise compare
boilerplate:
\begin{lstlisting}[language=bash]
...
\end{lstlisting}
This profile excludes the final no-object class, selects confident foreground
queries, and applies same-query class/box consistency across a weak photometric
view.  In the bounded 2026-08-01 10-image diagnostic, clean COCO mAP50:95 moved
from 0.000990 to 0.001188 and the shifted score from 0.000330 to 0.000396, with
no guard stop.  The result is a single-checkpoint local observation, not an
independent efficacy result; maturity remains Research and efficacy remains
\cmd{not\_established}.
...
\section{Implementation Notes: operational notes}
\begin{itemize}
