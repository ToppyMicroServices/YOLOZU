...
\ChapterAudience{Read this chapter when you need post-inference local correction and want to understand when Hessian refinement helps more than it hurts.}

\WorkflowOpening
{Apply bounded post-inference local refinement to an already produced predictions artifact.}
{Use this for offline analysis or controlled studies where small geometric corrections are worth extra compute.}
{A predictions artifact, dataset context, refinement settings, and an evaluation protocol for before/after comparison.}
{\cmd{python3 tools/refine_predictions_hessian.py --help} first, then run with explicit input, output, and report paths.}
{A refined predictions artifact plus a research report describing iterations, convergence, damping, and changed fields.}
{Validate both input and output artifacts, compare before/after metrics, and inspect whether updates stayed within the configured bounds.}
{Using refinement as a production default, missing dataset sidecars, unstable damping settings, excessive latency, or interpreting local correction as model retraining.}

\paragraph{Readiness classification.}
Hessian refinement is a \textbf{research-oriented} path in \yolozu{}.
Use it as a controlled post-inference study or offline correction stage, not as the default production lane.
See \path{docs/production_readiness.md}.


\section{Scope and relation to Test-Time Training (TTT)}
This chapter is strictly focused on per-detection numerical refinement executed after the primary inference phase. TTT, Tent, and MIM adaptation policies are covered in Chapter~\ref{chap:ttt}.
...
\begin{tikzpicture}[
  font=\small,
  box/.style={draw, rounded corners, align=left, inner sep=6pt, text width=3.8cm},
  inputbox/.style={box, fill=blue!8},
  processbox/.style={box, fill=orange!10},
  outputbox/.style={box, fill=green!10},
  arrow/.style={-{Latex[length=2mm]}, thick}
]

\node[inputbox] (pred) at (0,1.15) {\textbf{Detector output}\\initial offsets / pose-like fields};
\node[processbox] (res) at (6.0,1.15) {\textbf{Local residual}\\how wrong is this nearby solution?};
\node[processbox] (step) at (0,-1.85) {\textbf{Hessian-style step}\\small damped update};
\node[outputbox] (out) at (6.0,-1.85) {\textbf{Refined detection}\\same artifact family, smaller local error};

\draw[arrow] (pred) -- (res);
\draw[arrow] (pred) -- (step);
\draw[arrow] (res) -- (out);
\draw[arrow] (step) -- (out);
...
\section{Failure \texorpdfstring{$\rightarrow$}{->} local refinement example}
The typical use case is not ``the detector missed the object completely.''
It is the quieter case where the detector has already found the right object, but the local geometric state is still slightly wrong.
...
\node[box] (bad) at (0,0) {\textbf{Before refinement}\\The object is detected, but the local state is still off. Typical symptoms: center offset drift, biased depth, or a slightly wrong rotation.};
\node[box] (solve) at (6.5,0) {\textbf{Local solver view}\\Compute a residual around the current answer, estimate a damped second-order step, and keep the update bounded.};
\node[box] (good) at (13,0) {\textbf{After refinement}\\The object identity did not change; the same detection is simply better aligned in its local neighborhood.};

\draw[arrow] (bad) -- (solve);
\draw[arrow] (solve) -- (good);

\end{tikzpicture}
}
\caption{Concrete reading guide for Hessian refinement. This is not a ``new object found'' story; it is a ``same object, cleaner local alignment'' story. That distinction is why the method is especially relevant for pose-style outputs.}
\end{figure}
...
\begin{enumerate}
  ...
  \item Validate the predictions interface contract: \cmd{yolozu validate predictions reports/predictions_refined.json --strict}
\end{enumerate}

\subsection{Measured negative-control command}
Use the shared non-TTT artifact qualifier when a before/after result needs
three deterministic repetitions, stable COCO metrics, measured cost, hashes,
and an explicit promotion gate:
\begin{lstlisting}[language=bash]
./.venv/bin/python tools/qualify_artifact_research.py \
  --output-dir /tmp/yolozu-artifact-research
\end{lstlisting}
In the 2026-07-28 COCO128 run, all 1,280 detections in every repetition
stopped with \texttt{no\_signal}; the dataset does not provide the
per-instance depth/mask signal required by the current offset objective.
Metrics were unchanged, so the result remains \texttt{hold}. This records a
reproducible boundary and must not be described as evidence that Hessian
refinement improves model quality.
