...
\begin{tikzpicture}[
  font=\small,
  box/.style={draw, rounded corners, align=left, inner sep=8pt, text width=0.28\linewidth},
...
\node[inputbox] (pred) at (0,0.9) {\textbf{Detector output}\\initial offsets / pose-like fields};
\node[processbox] (res) at (6.2,0.9) {\textbf{Local residual}\\how wrong is this nearby solution?};
\node[processbox] (step) at (0,-1.5) {\textbf{Hessian-style step}\\small damped update};
\node[outputbox] (out) at (6.2,-1.5) {\textbf{Refined detection}\\same artifact family, smaller local error};
