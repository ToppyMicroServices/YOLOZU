...
The exporter records \path{weights_sha256}, projected exp parameters, preprocessing assumptions, and protocol metadata under \path{meta.extra.export_settings}.
...
This command generates a standardized predictions artifact, PNG visual overlays, and an HTML summary. The terminal output prints the predictions path, HTML path, overlays directory, and first overlay path so new users know exactly what to open next.
...
The exporters preserve raw framework box convention (\cmd{xyxy_abs}) in \path{export_settings.raw_output_bbox_format}, while normalizing contract output to \cmd{cxcywh_norm} for strict schema validation.
