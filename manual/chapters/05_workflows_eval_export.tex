...
\label{chap:evaluation-workflows}
...
yolozu eval-coco --dataset /path/to/yolo-dataset --predictions /path/to/predictions.json --output reports/eval_coco.json
\end{lstlisting}
Replace the placeholder paths with your dataset and predictions. This installed-package
path computes COCO metrics; \cmd{--dry-run} only checks conversion and report generation.

For a suite over several saved prediction artifacts, the repository also provides:
\begin{lstlisting}[language=bash]
python3 tools/eval_suite.py --dataset /path/to/yolo-dataset --predictions-glob '/path/to/predictions.json'
\end{lstlisting}
The suite helper requires a source checkout.
...
\subsection{Checked bring-your-own-project quickstarts}

\path{docs/byop_quickstarts.md} provides self-contained paths for
Ultralytics, Detectron2, MMDetection, and YOLOX. Each real path starts from a
user-owned model/config/checkpoint, emits a wrapped
\path{predictions.json}, runs strict predictions interface contract
validation, and writes the common \path{eval_report.json} through
\path{tools/eval_suite.py}.

New artifacts from these four quickstarts declare wrapper
\cmd{schema_version=1} and per-entry \cmd{schema_version=2} for both real and
dry-run artifacts. Strict validation therefore produces no legacy-version
migration warning. Older versionless artifacts remain accepted through the
compatibility path.

The document keeps two evidence classes separate:
\begin{itemize}
  \item a real command omits \cmd{--dry-run} and is accepted only when
  \path{execution_status=completed}, \path{runtime_executed=true}, and a
  positive \path{inference_calls} value are recorded;
  \item the CI schema smoke uses \cmd{--dry-run} and a 120-second timeout per
  source to check command syntax, wrapping, strict validation, and report
  generation without claiming third-party inference.
\end{itemize}

Each real block stops at the first failed command and requires a new output
directory, so a failed exporter cannot feed a stale artifact into validation or
evaluation. The first real run is capped at ten images with a ten-minute
operator stop condition. This is a troubleshooting budget, not a performance
guarantee. Framework, model, hardware, and input sizes determine actual runtime.

\path{tools/eval_suite.py} preserves bundled exporter settings from
\path{meta.extra.export_settings} in each report result. Equal report schemas
alone do not make metrics scientifically comparable: dataset identity, class
order, preprocessing, score/NMS settings, and bounding-box representation must
also match. The default user-supplied quickstarts have no named protocol and a
null protocol hash; null is not comparison evidence. Canonical parity requires
the same named protocol, successful fixed-condition checks, and the same
non-null protocol hash.
...
\begin{lstlisting}[language=bash]
# Minimal interface-contract example (always available)
...
For CI command-surface checks, prefer each exporter's \cmd{--dry-run} mode so schema and metadata interface contracts are validated without heavyweight runtime dependencies.
...
\subsection{Typed in-process validation and evaluation}

The stable Python surface is \path{yolozu.api}. It accepts path-backed or
in-memory predictions, returns typed results with \cmd{to_dict()}, and raises
machine-readable \cmd{APIError} subclasses instead of terminating the process.
Path resolution does not change the process working directory: callers use
absolute paths or provide an explicit absolute \cmd{base_dir}.
...
After deterministic dataset subsetting, predictions for known but unselected
images are excluded and counted. An image unknown to the full dataset still
fails. Strict validation is the default; \cmd{repair=True} is the explicit
in-process equivalent of CLI \cmd{--repair}. The canonical symbol list and
compatibility policy live in \path{docs/python_api.md}.
...
python3 tools/export_predictions_yolo_runtime.py --model yolo11n.pt --dataset data/yolo_wrapper --split val --max-images 8 --protocol nms_applied --wrap --output reports/pred_yolo_runtime.json
...
Without \cmd{--source}, \cmd{--max-images} truncates the ordered manifest
selection before those image paths are passed to the YOLO runtime. Zero
selected inputs and result-count mismatches fail before a new predictions
artifact is written. An explicit \cmd{--source} accepts one local image or a
directory expanded in sorted order, and cannot be combined with
\cmd{--max-images}.

Wrapped metadata records \path{source_mode}, \path{selected_inputs},
\path{selected_input_count}, and \path{result_count}. Completed non-dry
execution requires selected-input, result, inference-call, and predictions
cardinality to agree.
...
For teams already operating YOLOX experiments (\path{exp.py} + checkpoints), keep native inference and export directly to the YOLOZU predictions interface contract:
\begin{lstlisting}[language=bash]
...
When synthetic shards are generated externally (for example in \path{YOLOZU-synthgen}), \yolozu{} provides the interface-contract/eval side:
\begin{lstlisting}[language=bash]
...
For semantic-segmentation descriptors, export keeps the \path{dataset.json} interface contract while materializing \path{images/} and \path{masks/} into a lightweight external layout:
\begin{lstlisting}[language=bash]
...
To convert COCO-style detection results into the standardized YOLOZU predictions interface contract:
\begin{lstlisting}[language=bash]
