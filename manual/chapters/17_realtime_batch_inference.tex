...
It guides you to high-throughput deployment paths while keeping artifact interface contracts and drift detection in place.
}{
...
\WorkflowOpening
{Choose batch or real-time inference while keeping \path{predictions.json}, parity, and latency evidence explicit.}
{Use this when moving from offline evaluation toward ONNX Runtime, TensorRT, or streaming deployment constraints.}
{Model/export artifacts, input shape assumptions, preprocessing settings, runtime backend, and hardware notes.}
{\cmd{python3 tools/export_trt.py ...}, \cmd{python3 tools/build_trt_engine.py ...}, and backend export/parity commands with real paths.}
{ONNX or engine artifacts, metadata JSON, predictions artifacts, parity reports, and latency/FPS reports.}
{Confirm shape metadata, preprocessing, backend output format, parity tolerance, and hardware-pinned latency are all recorded.}
{Shape mismatch, hidden preprocessing drift, unsupported GPU/runtime libraries, unbudgeted TTT/refinement, or latency claims without hardware evidence.}
...
\begin{itemize}
  ...
  \item \path{docs/checkpoint_compatibility.md}
\end{itemize}
...
The public adapter, exporter, and backend suite use one fail-closed checkpoint
loader. Full compatibility requires every current model-state name and shape
to match after the documented uniform \cmd{module.} or \cmd{_orig_mod.} prefix
normalization. Reports include key differences, tensor-count and parameter
numel coverage, checkpoint/config SHA-256 values, and model identity. The
default deserializer uses \cmd{weights_only=True}; paths are descriptive, while
the hashes and model-state signature provide content identity. Use
\cmd{--allow-partial-checkpoint} only for intentional transfer or diagnostic
work; such results are labeled \cmd{status=partial} and are not
full-checkpoint evidence. Each tool removes its requested success artifact
before a real load, so an incompatible checkpoint cannot leave an older
prediction, export, or parity report at that path. An existing-ONNX
\cmd{--skip-onnx} run rejects \cmd{--checkpoint}, and the backend suite requires
\cmd{torch} in \cmd{--backends} whenever a checkpoint is supplied.
...
The exporter default and a protocol pin are different facts:
\cmd{tools/export_trt.py} defaults to ONNX opset 18, while the current
TensorRT/YOLO examples and self-hosted GPU smoke explicitly pin opset 17.
Record the selected opset in export metadata and compare only artifacts from
the same protocol. YOLOZU does not claim one universal TensorRT/CUDA version
pair. A qualification record must retain the exact environment or container,
GPU/driver context, TensorRT version, engine metadata, and parity report; see
...
In this repo, the main safeguards are implemented as \textbf{tooling and interface contracts} rather than as model-internal graph logic.
...
Note that real FPS validation is inherently \textbf{hardware-dependent}; CI runs can validate the harness and interface contracts,
...
\begin{itemize}
  \item \texttt{profiler\_available()} checks for \texttt{torch.profiler.profile}.
  \item \cmd{profile_inference(...)} profiles \cmd{adapter.predict()} calls and writes TensorBoard/\allowbreak Chrome-trace output.
  \item \texttt{profile\_callable(...)} profiles any callable and returns a text summary table.
\end{itemize}
...
The \texttt{yolozu.inference.torch\_export} module provides related compile-tracking and ONNX export utilities:

\begin{itemize}
  \item \texttt{compile\_for\_inference(model, ...)} --- wraps \texttt{torch.compile}, tracks backend, mode, fullgraph, and dynamic requests separately from actual execution, and fails closed through the first model call.
  \item \texttt{get\_compile\_evidence(model)} --- returns the attached JSON-safe requested, actual, runtime-counter, and failure evidence.
...
The \texttt{RTDETRPoseAdapter} integrates \texttt{compile\_for\_inference} via its \texttt{compile\_model}, backend, mode, fullgraph, dynamic, and explicit fallback constructor settings. A returned compile wrapper is only \texttt{pending\_first\_execution} after setup. It becomes \texttt{compiled} only after a model call completes. An unavailable API, invalid backend, setup error, lazy execution error, or \texttt{torch.\_dynamo.config.suppress\_errors=True} raises \texttt{TorchCompileError} by default because silent eager fallback cannot establish compiled evidence.

The export CLI requires \cmd{--wrap} with \cmd{--torch-compile}. In wrapped metadata, \cmd{inference} then \cmd{torch_compile} records the request under \cmd{requested}, the established state under \cmd{actual}, and available \cmd{torch._dynamo} process-counter deltas under \cmd{evidence}.

Eager fallback is opt-in through \cmd{--allow-compile-fallback}. It permits eager execution after a compile failure and records \cmd{actual.status=fallback}. Such an output is eager fallback evidence, not compiled evidence. A requested compile attempt removes an existing target before inference so an older successful JSON cannot survive a failed replacement attempt.
...
fast(sample_input)  # establishes first-execution evidence
...
\begin{itemize}
  ...
  \item compile request/actual status and settings, graph/counter evidence, any fallback, and enabled TTT/refinement/gates with bounds
\end{itemize}
