...
\node[box] (student) {\textbf{Student artifact}\\\path{predictions_student.json}};
\node[box, right=18mm of student] (teacher) {\textbf{Teacher artifact}\\\path{predictions_teacher.json}};
\node[box, below=14mm of student, xshift=33mm] (match) {\textbf{Match detections}\\same image, same class, IoU threshold};
\node[box, below=14mm of match, xshift=-38mm] (blend) {\textbf{Blend matched scores}\\controlled by \cmd{--alpha}};
\node[box, below=14mm of match, xshift=38mm] (inject) {\textbf{Optionally inject teacher-only detections}\\guarded by score / duplicate / count caps};
\node[box, below=16mm of match] (output) {\textbf{Distilled artifacts}\\\path{predictions_distilled.json}\\\path{distill_report.json}};
