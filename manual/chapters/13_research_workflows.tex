...
\subsection{Measured distillation and Hessian qualification}
For a concise, hash-bound comparison, run both non-TTT artifact lanes three
times and evaluate every output with the stable COCO evaluator:
\begin{lstlisting}[language=bash]
./.venv/bin/python tools/qualify_artifact_research.py \
  --output-dir /tmp/yolozu-artifact-research
\end{lstlisting}
The command refuses to replace an existing output directory and confines all
source artifacts, datasets, and configs to the repository. Its summary records
the source commit, input hashes, environment, per-run latency, normalized
prediction hashes, stable metrics, rollback, and the promotion decision.

The 2026-07-28 evidence is deliberately not a promotion result. Distillation
used a ten-image smoke fixture as the teacher and therefore establishes the
interface/evaluator path only. Hessian refinement received no depth/mask
signal on COCO128, so every attempted offset update was a \texttt{no\_signal}
negative control. Both lanes remain \texttt{hold}.
...
When using the in-repository trainer, use the contracted run mode to produce consistent artifacts. Chapter~\ref{chap:training} (Training and the Run Contract) describes this mode.
...
Use \cmd{yolozu calibrate} to generate calibrated prediction artifacts for side-by-side comparisons. Chapter~\ref{chap:calibration} (Score Calibration and Long-tail Adjustments) describes this workflow.
