...
\begin{itemize}
  ...
  \item \textbf{Missing optional extras}: Import errors for demo, ONNX Runtime, COCO, or backend utilities usually mean the base package was installed without the needed extra. Reinstall the specific lane with \cmd{yolozu[demo]}, \cmd{yolozu[onnxrt]}, or the extra documented by the workflow.
...
\end{lstlisting}
  ...
  \item \textbf{TensorRT environment}: TensorRT environment constraints are Linux/NVIDIA-centric. Use \cmd{gpu_validation_preflight.py} and the documented container workflow to keep CUDA/TRT/ORT compatibility pinned.
