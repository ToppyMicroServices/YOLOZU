...
\begin{itemize}
  ...
  \item \textbf{Container workflow expectations}: \path{.github/workflows/container.yml} runs build validation on \texttt{main} pushes, while image publication is gated to tag/manual runs.
  \item \textbf{Container trigger scope}: Container checks on \texttt{main} are restricted to container-related paths (workflow/deploy/packaging inputs) to avoid unnecessary runs.
  \item \textbf{Security scan backlog vs real regressions}: \path{.github/workflows/codeql.yml} and \path{.github/workflows/scorecard.yml} report repository posture, not just runtime test failures. If the build is green but code-scanning is open, inspect the cited file/line first and confirm whether the issue is an actionable reliability or supply-chain gap.
