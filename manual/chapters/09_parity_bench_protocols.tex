...
\cmd{onnx}, and \cmd{engine}. When those backends can execute, the command
records a real dataset-pass wall-clock measurement. When they cannot, it still
records skipped formats honestly rather than fabricating success.
...
At present, parity artifacts remain placeholders until the dedicated parity
gate is connected to \cmd{yolozu benchmark}. This is intentional: the command
...
\begin{itemize}
  \item broader format coverage such as \cmd{torchscript}, \cmd{openvino},
  \cmd{coreml}, \cmd{saved\_model}, \cmd{tflite}, \cmd{ncnn}, \cmd{rknn}, and
  \cmd{paddle}
  \item clearer task-specific benchmark expectations for
  \cmd{segmentation}, \cmd{classification}, and \cmd{obb}
  \item real parity artifact generation instead of the current placeholder
  \path{parity_<format>.json}
