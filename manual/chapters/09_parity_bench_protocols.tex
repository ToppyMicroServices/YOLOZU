...
metric family, expected metric keys, support level, and whether the task is an
Ultralytics-surface target or a YOLOZU-native extension.
...
\cmd{depth} & \cmd{depth} & \cmd{depth\_error} & documented partial & YOLOZU-native extension, not an Ultralytics parity claim. \\
...
\hline
\cmd{torch} & real detect path & local PyTorch + Ultralytics runtime & External runtime; verify your chosen runtime/model stack separately. \\
...
If the goal is to close the practical gap against the public Ultralytics docs
