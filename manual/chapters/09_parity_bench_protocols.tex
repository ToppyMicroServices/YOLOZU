...
\begin{itemize}
  ...
  \item \path{reports/export_settings_<format>.json} when the benchmark report
  itself is also written under \path{reports/}
...
Placement rule:
\begin{itemize}
  \item \path{export_settings_<format>.json} is written next to the path given
  by \cmd{--output}
  \item \path{predictions_<format>.json}, \path{eval_<format>.json}, and
  \path{parity_<format>.json} default under \path{reports/} unless their
  explicit override flags are set
\end{itemize}
