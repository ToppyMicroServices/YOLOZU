...
It assumes the presence of task-specific data fields (e.g., PNG masks for instance segmentation, and keypoint arrays for pose estimation); follow Chapter~\ref{chap:overview}'s operating rules by validating artifacts before scoring them.
}
...
\noindent Example result from the bundled tiny COCO subset: \path{image_id=407083},
...
\begin{lstlisting}[language=bash]
python3 tools/prepare_keypoints_dataset.py   --source /path/to/export   --format auto   --out /path/to/keypoints_dataset
\end{lstlisting}
