...
For a visible, reproducible compare on real images, \yolozu{} also provides a dedicated instance-seg TTA demo using \texttt{torchvision} Mask R-CNN. It applies a deterministic corruption to a COCO image, runs raw inference, then reruns with horizontal-flip TTA and selects a case with a clear metric improvement:
\begin{lstlisting}[language=bash]
yolozu demo instance-seg-tta --run-dir reports/demo_instance_seg_tta
\end{lstlisting}

\noindent Example result from the bundled tiny COCO subset: \path{image\_id=468577},
\path{brightness} severity \path{5}, with \path{tp 1->2}, \path{fp 1->0}, and \path{fn 1->0}.
...
\begin{figure}[h]
  ...
  \begin{subfigure}[t]{0.48\textwidth}
    ...
    \caption{TTA On (hflip)}
  \end{subfigure}
  \caption{Instance segmentation overlay comparison on the same corrupted real COCO image. Green denotes ground truth, red denotes predicted masks, and increased overlap appears yellow.}
\end{figure}
