...
\ChapterAudience{Read this chapter when you need to discover declared tools, understand their inputs and outputs, or build automation from the registry.}

\section{Boundary With the Maintenance Chapter}
This chapter explains how to \emph{read and use} the tool registry: what the
manifest contains, how to query it, and how automation should treat declared
entrypoints. Chapter~\ref{chap:manual-maintenance} owns the \emph{maintenance workflow}: changing tool
behavior, updating \path{tools/manifest.json}, syncing packaged copies, and
keeping docs/manual examples aligned.
...
\section{Authority for Discovery and Execution}
For CLI capabilities and tool I/O behavior, \path{tools/manifest.json} serves as the \textbf{authoritative source of truth}. Treat prose examples as derived guidance: use the manifest and the tool's \cmd{--help} output when wiring automation.

When discovery surfaces disagree:
\begin{itemize}
  \item \textbf{Behavioral Truth}: Resides in the code and the manifest.
  \item \textbf{Operator Action}: Prefer the declared manifest entry and verify the tool's \cmd{--help} output.
  \item \textbf{Maintainer Action}: Use Chapter~\ref{chap:manual-maintenance} to update the manifest, packaged copy, docs, and manual together.
...
\section{Interface Contracts Referenced by the Manifest}
...
# Identify tools that consume the 'predictions_json' interface contract
python3 - <<'PY'
import json
with open("tools/manifest.json", "r", encoding="utf-8") as f:
    manifest = json.load(f)
...
\begin{itemize}
  ...
  \item \textbf{Interface Contracts and Schemas}: \path{docs/schemas/*.schema.json}
...
\section{When You Need to Change the Registry}
Do not treat this chapter as the maintenance checklist. If you add a flag,
change an output path, add a public tool, or alter manifest metadata, follow
Chapter~\ref{chap:manual-maintenance}. That chapter is the canonical update procedure for:
\begin{itemize}
  \item changing tool behavior and \cmd{--help} output,
  \item updating \path{tools/manifest.json} and the packaged manifest copy,
  \item syncing docs/manual examples, and
  \item running the manifest and docs-reference quality gates.
\end{itemize}
...
\begin{itemize}
  ...
  \item \path{CHANGELOG.md} must contain a matching \texttt{\#\# [X.Y.Z] - YYYY-MM-DD} section
  \item \path{CITATION.cff} must carry the same version and release date
  \item \path{tools/manifest.json} and its packaged copy must be byte-identical, with exact-version examples classified as current or evidence-backed historical pins
\end{itemize}
...
bash release.sh
\end{lstlisting}
Publishing the GitHub Release is the single automatic trigger for both the PyPI and
manual DOI workflows. The helper does not dispatch the manual DOI workflow again.
Before writing, it validates the current metadata and the complete next-release
plan. It then synchronizes the package version, changelog heading/date, citation
version/date, and examples explicitly marked as current-release-coupled in one
rollback-protected operation. Evidence-backed historical manifest examples are
kept unchanged. The tag helper repeats this validation and stops before tag or
GitHub Release creation if any surface disagrees.
...
\end{lstlisting}
The check and dry-run reports list mismatches without changing release metadata;
the dry-run also records the full next-version file plan.
...
\end{itemize}
Manual \cmd{workflow_dispatch} is limited to first-time setup, recovery, sandbox, or
draft use. Runs for the same version are serialized, and a version already present
under the configured concept record returns \texttt{already\_published} before a new
...
\begin{itemize}
  ...
  \item \texttt{YOLOZU\_MANUAL\_CONCEPTRECID} (required for automatic updates after the first manual record)
\end{itemize}
The first manual deposition is a recovery-style manual run with
\texttt{create\_first\_deposition=true}; release-triggered runs never create it
implicitly.
