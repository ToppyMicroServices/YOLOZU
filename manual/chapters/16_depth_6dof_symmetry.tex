...
license boundary, and current evidence gaps are pinned in
...
metric scope.  The tracked fixture has COCO ground truth for boxes and polygon
masks, but keypoint/depth/pose6d sidecars are deterministic heuristics.  The
staged runner currently reports bbox mAP after every stage rather than
task-native metrics for the latter four stages, so the qualification remains
\cmd{hold} and Experimental.
The clean-source bounded result is recorded in
\cmd{reports/finetune\_lane\_evidence\_2026-07-29.md}.
