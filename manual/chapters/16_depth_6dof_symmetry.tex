...
\begin{itemize}
  \item \cmd{abs\_rel}, \cmd{sq\_rel}, \cmd{rmse}, and \cmd{rmse\_log}
...
\cmd{--align median\_scale} mode is appropriate for relative monocular depth
...
\begin{itemize}
  \item records the backend-specific depth artifact under \path{predictions\_<format>.json}
  \item evaluates each artifact with \path{tools/eval\_depth.py}
...
\begin{itemize}
  ...
  \item evaluates each artifact with \path{tools/eval\_keypoints.py}
...
\begin{itemize}
  \item copies each backend \path{predictions.json} artifact into the benchmark artifact layout
  \item evaluates each artifact with \path{tools/eval\_pose.py}
...
\begin{itemize}
  \item pose core metrics such as \cmd{pose\_success}, \cmd{rot\_deg\_mean}, and \cmd{trans\_l2\_mean}
  \item optional depth error when the predictions expose depth/translation
  \item optional CAD-point metrics such as \cmd{add\_mean} and \cmd{adds\_mean}
\end{itemize}
