...
\cmd{yolozu benchmark} & Parity-target benchmark entrypoint. Runs real backend orchestration for \cmd{torch}/\cmd{onnx}/\cmd{engine} when artifacts and runtimes exist; for \cmd{--task keypoints}, \cmd{--task depth}, and \cmd{--task pose6d}, it uses artifact-backed real eval/parity lanes rather than pretending YOLOZU executed the backend inference itself. Key knobs include \cmd{--protocol}, backend-specific model overrides (\cmd{--torch-model}, \cmd{--onnx-model}, \cmd{--engine-model}), \cmd{--latency-source auto|artifact\_eval}, keypoints-specific \cmd{--keypoints-parity-*} thresholds, depth-specific \cmd{--depth-mask}/\cmd{--depth-align}, and pose-specific \cmd{--pose-parity-*} thresholds. \\
...
\path{python3 tools/benchmark_model.py} & Repository wrapper for \cmd{yolozu benchmark}; useful when backend-specific benchmark behavior, including the artifact-backed keypoints/depth/pose6d benchmark/parity lanes, must also be tracked in \path{tools/manifest.json}. \\
