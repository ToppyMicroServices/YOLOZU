...
\cmd{yolozu export-dataset yolo|coco|kitti} & Exports a YOLOZU dataset wrapper into a standalone YOLO training layout (\path{data.yaml} + \path{images/}/\path{labels/}), a standard COCO layout (\path{images/<split>/} + \path{annotations/instances\_<split>.json}), or a KITTI-style layout (\path{image\_2}/\path{label\_2}/\path{ImageSets/Main}). \\
