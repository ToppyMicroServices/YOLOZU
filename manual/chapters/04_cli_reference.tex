...
\cmd{yolozu demo pose} & 6D pose demo (chessboard/ArUco by default; DenseFusion backend requires CUDA + large downloads). Writes overlay under \path{demo\_output/pose/}. \\
...
\cmd{yolozu demo train} & Training demo (MNIST fine-tune; bounded by \cmd{--max-steps}; writes under \path{demo\_output/train/}). \\
