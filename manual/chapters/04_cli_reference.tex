...
\cmd{yolozu train} & Initiates YAML/JSON config-driven RT-DETR training by default. With \cmd{--external-backend yolox|ultralytics|hf-detr}, it switches to the external training lane while keeping the same top-level train surface. Post-train ONNX export is attempted on CPU for portability, even if training ran on CUDA or MPS. \\
