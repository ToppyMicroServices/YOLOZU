...
\cmd{yolozu train} & Initiates YAML/JSON config-driven RT-DETR training by default. With \cmd{--external-backend yolox|ultralytics|hf-detr}, it switches to the external training lane while keeping the same top-level train surface. Post-train ONNX export is attempted on CPU for portability, even if training ran on CUDA or MPS. All supported lanes now converge on a shared top-level training summary interface contract. \\
...
\path{python3 tools/support\_external\_training.py} & External training support utility with a fixed 3-layer interface contract. The primary lane is \cmd{train-yolox} (Apache-2.0-friendly external YOLOX bridge); \cmd{train-ultralytics} remains an optional bridge. Shared dataset conversion, ONNX export templates, and prediction canonicalization stay on the same surface. Legacy alias: \path{tools/support\_yolo\_detr.py}. \\
\bottomrule
\end{longtable}
\normalsize
