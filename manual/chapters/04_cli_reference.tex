...
\cmd{python3 tools/run\_reference\_adapter\_regression.py} & Runs RT-DETR reference-adapter regression with profile-aware micro/full modes, interface contract gates (hard), behavior/parity gates (warn/hard), provenance snapshot capture (\cmd{--capture-provenance}), matrix baseline layout (\cmd{--baseline-layout matrix}), and lock/hash enforcement (\cmd{--runtime-lock}, \cmd{--enforce-runtime-lock}, \cmd{--enforce-weights-hash}, expected hash flags). \\
