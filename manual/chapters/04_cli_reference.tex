...
\cmd{yolozu benchmark} & Phase-1 Ultralytics-parity benchmark entrypoint. Writes a stable benchmark report plus per-format placeholder artifacts, and reports skipped backends explicitly instead of fabricating success. \\
...
\cmd{python3 tools/benchmark\_model.py} & Repository wrapper for \cmd{yolozu benchmark}; useful when benchmark-mode behavior must also be tracked in \path{tools/manifest.json}. \\
