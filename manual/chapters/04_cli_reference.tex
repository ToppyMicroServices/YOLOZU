...
\cmd{yolozu train} & Initiates YAML/JSON config-driven RT-DETR training by default. With \cmd{--external-backend yolox|detectron2|mmdetection|mmpose|mmseg|ultralytics|hf-detr}, it switches to the external training lane while keeping the same top-level train surface. Detectron2 covers bbox / instance segmentation / keypoints, MMDetection covers bbox / instance segmentation, MMPose covers keypoints, and MMSeg covers semantic segmentation when those tasks already live in external stacks. Post-train ONNX export is attempted on CPU for portability, even if training ran on CUDA or MPS. All supported lanes now converge on a shared top-level training summary interface contract, and external lanes also write a fixed wrapper-owned run bundle under \path{work\_dir/}, including \path{reports/export\_handoff.json}, \path{reports/eval\_handoff.json}, \path{reports/parity\_handoff.json}, and \path{reports/training\_registry\_entry.json}. OpenCV DNN and ONNX Runtime are inference/export runtimes, not training backends. \\
\cmd{yolozu train-orchestrate} & Plans or executes a small multi-backend training batch from one JSON spec and writes \path{training\_orchestration\_report.json}. When \cmd{--registry-out} is set, executed runs are also appended to a shared JSONL training registry. This is repository-side experiment orchestration, not a cluster scheduler. \\
...
\path{python3 tools/support\_external\_training.py} & External training support utility with a fixed 3-layer interface contract. The primary lane is \cmd{train-yolox} (Apache-2.0-friendly external YOLOX bridge); \cmd{train-detectron2}, \cmd{train-mmdetection}, \cmd{train-mmpose}, and \cmd{train-mmseg} cover common external vision stacks; \cmd{train-ultralytics} remains an optional bridge. Shared dataset conversion, ONNX export templates, and prediction canonicalization stay on the same surface. Legacy alias: \path{tools/support\_yolo\_detr.py}. \\
\bottomrule
\end{longtable}
\normalsize
