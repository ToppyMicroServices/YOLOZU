...
For record-based training, \cmd{--records-json} supplies the training records and \cmd{--val-records-json} supplies validation records. Both files may be a JSON list of records or an object with an \path{images} list. Use this path when the dataset cannot be cleanly represented as one YOLO-style split directory.
