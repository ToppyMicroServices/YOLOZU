...
\begin{itemize}
  \item \textbf{RT-DETR / DETR-family}: the in-repo reference lane. Use AdamW,
  separate backbone/head learning-rate groups, gradient clipping, warmup, EMA,
  and NMS-free evaluation/export assumptions by default. See
  \path{configs/examples/train_rtdetr_stable.yaml}.
  \item \textbf{YOLO-family}: external YOLO lanes such as YOLOX. Preserve
  letterbox preprocessing, SGD-style optimization defaults, and NMS-applied
  export/evaluation assumptions. See
  \path{configs/examples/finetune_external/yolox_s_finetune_smoke.py}.
\end{itemize}

Do not copy one family's defaults into the other. A DETR-family run should not
silently inherit YOLO letterbox/NMS assumptions, and a YOLO-family run should not
silently inherit DETR AdamW/head-backbone group assumptions unless the backend
configuration explicitly says so.

\subsection{Training data flow}
Training data uses one common route before it reaches a backend:
\begin{center}
\textbf{raw dataset}
\(\rightarrow\)
\textbf{DatasetAdapter}
\(\rightarrow\)
\textbf{YOLOZU Dataset Contract}
\(\rightarrow\)
\textbf{TrainingBackend}.
\end{center}

The DatasetAdapter handles source formats such as YOLO \path{data.yaml},
\path{dataset.json} wrappers, COCO JSON, and SynthGen shards. The YOLOZU Dataset
Contract is the shared training boundary: records prefer \cmd{xyxy_abs} storage
and expose \cmd{xywh_abs} and \cmd{cxcywh_norm} adapter views when image size is
available. YOLO-family backends consume the \cmd{cxcywh_norm} view; DETR-family
backends consume \cmd{xyxy_abs}. The training summary records this boundary in
\cmd{training_data_flow}.
...
In other words, users do not need to manually classify the incoming dataset family, but the reference trainer itself still consumes a train-ready YOLOZU/YOLO-style result of that preflight/import step. Semantic-segmentation descriptors and SynthGen intake shards are import/evaluation inputs unless they are converted into train-ready image and label records. The \cmd{doctor import} report includes \path{reference_trainer.direct_train_ready} and \path{reference_trainer.train_ready_after_migration} so automation can distinguish direct training inputs from native sources that need migration first. The train-specific \cmd{doctor train-dataset} route performs the same check and emits next commands without launching training. It also recognizes explicit \cmd{classification}, \cmd{obb}, \cmd{depth}, and \cmd{pose6d} source families. Classification folders/manifests and OBB labels are recognized intake interface contracts for external training lanes, not direct RT-DETR reference-trainer inputs. Depth and pose6d are direct only when normalized bbox records include the required sidecars. Through the public CLI, \cmd{python3 -m yolozu train <config> --dataset-root <wrapper>} forwards the dataset root to the RT-DETR reference trainer.

Dataset preflight fails closed when the selected split contains no records. Direct-train readiness uses the same strict record validation as \cmd{yolozu validate dataset}: images must be readable and normalized bbox values must be valid. Stored values are not clipped or rewritten. A tolerance of \cmd{1e-6} applies only to floating-point round-off in derived bbox edges. The machine-readable \path{records.validation} object records the policy, checked and total counts, whether the scope is \cmd{all} or \cmd{first_n}, and all reported errors and warnings.

The authoritative source/target table is \path{docs/dataset_processing_matrix.md}. A deterministic multi-task subset is valid only when selected images and labels retain referenced mask, depth, CAD, intrinsics, keypoint, and object-pose sidecars. The subset helper records per-file SHA-256 values and replaces only an ownership-marked output directory; it refuses symlink, protected, and unowned replacement targets.
...
Separate COCO paths can be checked without a synthetic root:
\begin{lstlisting}[language=bash]
python3 -m yolozu doctor train-dataset \
  --from coco-instances \
  ...
  --output -
\end{lstlisting}
The \cmd{--instances} and \cmd{--images-dir} options are required together. This source remains a migration input; the report emits the matching import command only after strict validation succeeds.
...
\subsection{Artifact interface contract: required vs optional}
...
The 2026-07-30 installed-runtime qualification executed real Ultralytics,
Transformers DETR, and Detectron2 training in two Python/Torch environments,
then exported and evaluated their predictions interface contract artifacts.
YOLOX, MMDetection, MMPose, MMSeg, and TAO emitted structured runtime failures
on the tested macOS CPU host.  The matrix records wall time and peak RSS and
does not count config projection as training.  Results remain Experimental and
\cmd{hold}; see
\path{reports/external_runtime_evidence_2026-07-30.md}.

A separate compatible Linux/CUDA qualification at source commit
\cmd{806496d} completed non-dry YOLOX, MMDetection, MMPose, MMSeg, and
NVIDIA TAO training in two independent Tesla T4 runs.  Both runs recorded
checkpoints, launcher resource use, and valid structural
resume/export/eval/parity handoffs.  The source pins, dataset tree, and all
fail-closed gates reproduced, while checkpoint hashes differed.  This
establishes compatible-host runtime availability and structural handoff
reproducibility, not checkpoint byte determinism or training quality.  The
lanes remain Experimental and \cmd{hold}; see
\path{reports/external_runtime_compatible_host_evidence_2026-07-30.md}.
...
For the plain-language intuition and the difference between LoRA and QLoRA, see Chapter~\ref{chap:continual-learning} before tuning these knobs.
...
LoRA checkpoints retain the base and adapter tensors in one model state. Reload
the same model architecture, inject the same rank, alpha, and target, and then
load the checkpoint. The regression suite verifies an adapter-only update,
checkpoint save, exact prediction parity after reload, and one additional
adapter-only update.
...
The backbone output interface contract strictly mandates three feature maps: \texttt{[P3, P4, P5]} with corresponding strides of \texttt{[8, 16, 32]}. Each level is independently projected to the transformer's \texttt{d\_model} dimension via 1x1 convolutions, then processed by the RT-DETR-style FPN/PAN + hybrid encoder path.
...
Optional query-level graph refinement is configured in the same model JSON with
\cmd{model.graph_refine}. The default is \texttt{\{"mode": "none"\}}. Set
\texttt{mode} to \texttt{gcnv2} or \texttt{gcnv3} to add a residual graph
refinement stage after the RT-DETR decoder and before the prediction heads.
Packaged examples are available as \path{rtdetr_pose/configs/gcnv2.json} and
\path{rtdetr_pose/configs/gcnv3.json}.
...
\begin{itemize}
  ...
  \item \cmd{make_amp_context(device_type, dtype, enabled)} --- creates a paired \cmd{(autocast_factory, GradScaler|None)} tuple. A \texttt{GradScaler} is only returned for \texttt{float16} on CUDA; for \texttt{bfloat16}, MPS, or CPU the scaler is \texttt{None}.
\end{itemize}
