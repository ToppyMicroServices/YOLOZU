...
\begin{itemize}
  ...
  \item \textbf{SynthGen Intake Contract}: \path{docs/synthgen_contract.md}
\end{itemize}
...
\node[box, right=22mm of coco] (wrapper) {\textbf{Wrapper / mapping}\\
\path{dataset.json}\\
\path{classes.json}};
\node[box, right=22mm of extpred] (pred) {\textbf{predictions.json}\\
schema-valid\\
relocatable paths};
...
\draw[arrow] (coco) -- node[midway, right]{\scriptsize convert labels} (wrapper);
