...
The installed package contains three non-promoted, unbound adaptive Candidate bundle
records matching the existing model zoo, plus one bound Torchvision Mask R-CNN
Candidate that consumes an exact user-prepared local safetensors artifact. The bounded loader validates this registry and lifecycle
...
command is installed. It alone may measure the exact bound Candidate for an intended
Experimental request with the code-owned local COCO evaluator. Candidate remains
unavailable to ordinary routing. Its \cmd{--help} documents
the exact-bundle, timeout, pinned-input, and managed-output interface contract.
The first retained 16-image run returned \cmd{hold} because its preregistered
p95 latency limit was exceeded. No activation, support assignment, or promotion
was created from that report.
...
but the Candidate-only registry, held screenings, empty public evidence stream,
and unbound packaged bundles make the default recommendation abstain with
\cmd{maturity\_disallowed} and processing
