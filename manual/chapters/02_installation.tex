...
On macOS, you can typically run CPU-bound tooling (validation, evaluation, and ONNX Runtime CPU export). MPS is supported only when the local Torch runtime reports \cmd{torch.backends.mps.is_available() == true}; otherwise treat the machine as CPU-only for \yolozu{} purposes. TensorRT execution is generally Linux/NVIDIA-centric.
